...
const CE_I18N = {
  en: {
    ...
    resolvedTo: (p) => `resolved to ${p}`, pickRoot: "choose the repository folder",
    ...
    // the audit trail's face (O50): the same records `ce erase --log`
    // prints, read from .ce/erase-log.ndjson — never written here
    eraseLog: "audit log", readingLog: "reading the audit log…",
    logHead: (n, k) => `${n} applied record(s), ${k} unreadable line(s)`,
    logEmpty: "the audit log exists but holds no record",
    logAbsent: "no audit log yet — nothing has been applied here",
    logUnreadable: (n) => `unreadable line ${n}`,
    ...
    axisNames: ["geometry", "naming", "mixing", "misplaced", "docs", "stale-docs", "redundancy", "modularity"],
  ...
  zh: {
    ...
    commits: "提交数", load: "加载",
    resolvedTo: (p) => `已锚定到 ${p}`, pickRoot: "选择仓库目录",
    ...
    eraseLog: "审计日志", readingLog: "读取审计日志…",
    logHead: (n, k) => `${n} 条已执行记录，${k} 行不可读`,
    logEmpty: "审计日志存在但没有记录",
    logAbsent: "尚无审计日志——这里还没执行过擦除",
    logUnreadable: (n) => `第 ${n} 行不可读`,
    ...
    axisNames: ["几何", "命名", "混流", "错位", "文档", "陈旧文档", "冗余", "模块度"],
