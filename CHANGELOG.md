...
**O42+O20** 类 glob 与 exclude 列表同一解析器（`scan/globs.rs`：gitignore 文法、`/` 分隔、`\` 为转义、前导 `!`/`#` 按名拒绝、`dir/` = 目录内容）——此前类走 globset 方言，同一串在两张表里两种读法；`[[rules.class]]` 的 `dir/` 自此选中目录内容（旧读法下零命中），启类仓库经此切换分数不可比、须具名重立（自仓不启类，刻度不动）。
