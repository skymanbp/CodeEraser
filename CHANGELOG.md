...
**无默认档位变更。** 语言扩展 v2.30 步 4 Lua + R 提交 B′：判分与精度册（2026-09-25；判决代码零改动、分数不动；四份档在提交 B〔642e919〕之上生成——顺序门要每份档的 `generated_from` 严格晚于每张审阅表的首个提交，Lua 第二代的审阅表是 f473c39，与阶梯同一棵树上生成的档过不了它，所以阶梯先落地、档随本提交）：

- **精度册**（`contracts/eval/lang-precision-luarocks-v2.json`、`lang-precision-koreader-v2.json`、`lang-precision-stringr-v1.json`、`lang-precision-covid19model-v1.json`；判分 `it/eval_lang_parts/score.rs`、核对 `precision.rs`、CI 门 `it/eval_lang_precision.rs`；登记 `docs/EVAL-SET-LANGS.md` 两语言各一节「判分」；考题表 Lua 与 R 的 `scored` 翻为 true）：Lua：luarocks 10/10、koreader 75/75，整体 85/85 = 1.000（门 0.90），站内真值 88 道答中 85 道——三道 missed 都是搜索目录不在文本里的情形（luarocks 两处 `compat53.module` 前言的 `vendor/` 只在 `GNUmakefile` 写的 `LUA_PATH` 里，声明 `[graph.search_roots] lua = ["vendor"]` 即答对；koreader 一处 `spec/unit/` 由未检出子模块里的测试运行器配置加进路径）；宇宙台账 luarocks 702 站点解出 79.8 %、koreader 5,058 解出 84.1 %，首级占比 99.8 % / 96.5 % 过了 0.80 的触发线，两份档各带书面处置（`r0_disposition`：Lua 的第一级就是 `require` 的整个搜索，占比复述的是站点构成）。R：covid19model 18/18（`source` 15 + `library(covid19AgeModel)` 3），stringr 两道样本都是外部包、站内真值为零，精度无定义，包的读法由宇宙台账检验——stringr 55 站点解出 100 %（`tests/testthat.R` 的 `library(stringr)` 答包根、54 道 External），covid19model 1,697 解出 99.8 %（R1 47、R2 77、R3 1,570，3 道拒答是 `source` 指向语料里没有的文件）；首级占比 0 % / 2.8 %，不触发。审阅者记下的候选漏检 5 + 6 条逐条核实（luarocks 三条 compat53 前言检测器都读到、阶梯同答 out_of_scope；koreader 两条与 covid19model 三条 `system("Rscript …")` 那一行没有字面实参的站点；covid19model 另三条读到并解出），没有一条是真漏。
- **计划**：计划书横幅与 §6 T 轨步 4 记「已交付」（六个提交：A / A′ / A2 / A2′ / B / B′），设计册 §13 行 4 同改；步 4 至此收口，下一步 = 步 5 HTML。
- 门：主 check 944 / dedup 59 / scan 85 warn 0 fail（deadcode 0 / docdup 0 / erase 0），子仓 982 / 119 / 43 warn 0 fail；lib 395 / it 435 绿 0 红 (15 ign)——判决零改动：重立后同一棵树 check 944 不动（size 轴 83）；四份档入普查宇宙，册 13 自仓行 U 1151 → 1155

**无默认档位变更。** 语言扩展 v2.30 步 5 HTML 提交 A（2026-09-26；HTML 从纯尺寸臂升为文档类判决语言，判决宇宙再多一种语言，**分数与 1.7.4 不可比**；同批三处根修挪了走查读的文件集与 Java / Lua / R 的引用答案——既有语言的判决变化见对拍）：

- **HTML 判决**（`Lang::Html` 翻位，tree-sitter-html 0.23.2 接进 `Lang::grammar`，第十二套文法）：文档语言，与 Markdown 同类——没有函数单元，不算克隆指纹（`Lang::fingerprints()` 排除它：叶子不带标识符，任意两页都会哈希成一对克隆）。三个读者共用 `scan/html.rs` 的标签读法（元素名与属性名不分大小写，值取引号内或裸值，无值属性读作空串）：带 `id` 的元素是一节（`fourclass/units.rs`，键 `#id`，随元素嵌套，与 Markdown 标题同为公开）；引用站点是（元素，属性）对（`graph/sites/html.rs` 一张十八行的表：`a` / `area` / `base` / `use` / `link` 的 `href` 与表单的 `action` 指页面，`script` / `img` / `source` / `iframe` / `embed` / `track` / `video` / `audio` 的 `src`、`video` 的 `poster`、`object` 的 `data` 与 `img` / `source` 的 `srcset` 指资源——`srcset` 每个候选一个站点；`<link>` 按 `rel` 分：样式表、图标、预载等是 `link_asset`，其余是页面）；五个站点种类追加进 `store::KINDS`（23 个，`GRAPH_REV` 不动：只落在索引从没存过的文件上）；边：`href` / `action` 指页面为 doc_link、指代码为 doc_ref，`src` / `srcset` / `link_asset` 一律 asset。HTML 的站点在阶梯落地（提交 B）前一律 `unsupported` 入台账。docdup 多一个段种类 `html_text`（`docdup/html.rs`：块级元素的文字成段，行内 `code` 与 `pre` / `textarea` 掩掉，`script` / `style` 不进、入台账；注释掩掉不成段，所以写在 HTML 注释里的 `ce:allow(docdup) -- 理由` 照样生效）；提及的域不含 HTML 的 id（锚点不是标识符，RG9，同 Markdown 标题）；入口名加 `index.html` 与 `404.html`（服务器按名字给的页）；评分的文档轴按 Markdown 与 HTML 两种文件计。
- **`DOCDUP_REV` 5 → 6**：`KIND_NAMES` 追加 `html_text`（码 3）；骨架表加 Javadoc / Doxygen / roxygen / LDoc 标签，共 43 条（`@return` 覆盖 `@returns`、`@throw` 覆盖 `@throws`，`@exception` 是 Javadoc 的同义词，Doxygen 命令写成 `\` 与 `@` 同读）；Lua 与 Haskell 的 `--` 注释前缀在分隔线规则之后剥掉（单个 `-` 仍是列表符号）；两张标记表各改成一条 `|` 分隔的字面量（一串字符串字面量在查重门眼里是同一个记号的重复）。五份冻结语料按 `docs/EVAL-SET.md` 的复活协议重冻结：只有 zod 一处移动——`packages/zod/src/v4/classic/tests/to-json-schema.test.ts` 第 7 行 `/** @see … */` 成了骨架行，台账 skeleton_line 13 → 14，live 段与哈希不变；自仓精度回显 259 → 260 是 96702a6 具名改签 tokens.rs 时欠下的补账；oracle 与普查不动（`docs/EVAL-SET-M5-3.md`）。
- **走查的构建产物规则**（`scan/outputs.rs`）：`target/ build/ dist/ dist-newstyle/` 不再按名字在任何深度排除，只在旁边站着产出它的工具的项目文件时才算产物（一行一个目录名：Cargo、Maven、sbt、Leiningen 的 `target/`；Gradle、setuptools、Flutter 的 `build/`；`package.json`、setuptools 与 Cabal 旧式的 `dist/`；Cabal 的 `dist-newstyle/`；约定而非默认的输出目录如 CMake 的 `-B build` 不入表，由 `.gitignore` 或 ce.toml `exclude` 声明）。luarocks 把构建后端放在 `src/luarocks/build/`（模块 `luarocks.build`），KOReader 的插件把模块放在 `target/`——旧规则把它们从每一次测量里藏掉了，引用图也不例外。走查在目录门口剪枝，`Scope` 同问（守卫对还没建出来的文件照样答），`ce eject` 找残留时同读。
- **Java 源集与本单元**（`graph/ladder/java_sets.rs`）：Maven / Gradle 的标准布局 `<模块>/src/<集>/java/`——`main` 集只看得见 `main` 集（构建从不把测试产物放上它的类路径），其余集看得见自己与 `main`；拆在两个集里的包答引用文件自己源根里的那一半。文件自己声明的名字（`C.java` 里的 `import static a.b.C.Nested.*`）不指向别的文件：新拒答原因 `own_unit`（JLS 7.3），与检测器早已丢掉的、指向本单元的 `type_ref` 同理。
- **Lua 引用文件自己的目录**（`graph/ladder/lua.rs`）：`require` 多找引用文件自己的目录——标准路径的 `./?.lua` 按 R2 读路径的同一读法（从文件自己的目录或仓根运行），也是插件加载器（`string.format("%s/?.lua;%s", plugin_root, package.path)`）与按 spec 目录加路径的测试运行器替 `require` 做的事；它是多一个搜索目录，不是首选：插件里与仓根各有一个 `util` 就是两个文件，ambiguous_root。
- **考题只问走查读的文件**（子仓 `it/eval_lang_parts/`）：站点宇宙按产品自己的走查取文件（走查拒读的被追踪文件只计 `walk_refused`，不出题）；判分时真值指向拒读文件的按站外判，审阅者原话另记 `audit_truth`；精度册多一节 `walk`（拒读的文件与它们的站点计数），schema `ce.eval-lang-precision/1.1.0`；走查的读法独立成 `walk.rs`。
- **精度册的新鲜度**：每个提交的门 `lang_docs_answer_the_code_they_name`（生成时树干净、生成提交在本历史里、此后没有提交碰过阶梯或答案依赖的代码 `ANSWERED_BY`），发版前的重放 `eval_lang_parts::replay`（`#[ignore]`，读全部钉住克隆逐键比对；`docs/RELEASE.md` §0）。重放在旧档上实跑，抓出 koreader 的行、摘要、台账三处移动。
- **Java / Lua / R 的六份精度册退役待重判**：本提交挪了它们的答案所依赖的代码（上面三条），六份档删档、考题表的 `scored` 翻回 false，等 HTML 阶梯落地后与 HTML 的一起在干净的树上重生成一次。精度册的两条篡改门改在神谕档上做（`eval_lang_parts/tamper.rs` 的 `oracle_precision`：只用冻结的样本、审阅表、宇宙拼出、每道题都答它的真值——核对只把档对照冻结输入、从不对照阶梯，所以神谕档过得了核对），篡改门因此不依赖哪门语言此刻判了分；两处反向探针（去掉比率断言、去掉「拒读文件须在宇宙内」）各自转红，还原后逐字节相等。
- **HTML 考题冻结**（设计册 §11；第三份语料是用户的裁定）：本仓 `codeeraser`@`d4b7f1f`（11 页 309 站点）、`h5bp/html5-boilerplate`@`b659733`（2 页 6 站点，`dist/` 的两份构建产物旁边站着 `package.json`，走查拒读）、`mdn/learning-area`@`dbed6bc`（269 页 453 站点，前两份一个表单、一个 `srcset` 都没有）。共 768 站点（`href` 336、`src` 222、`link_asset` 197、`srcset` 7、`action` 6）；100 道主样本（`href` 34、`src` 27、`link_asset` 26，`srcset` 与 `action` 取尽），备用 60 道。对拍夹具加 html 行（本仓五页，只作站点复核）。
- **对拍**（旧 = d4b7f1f 编出的程序，新 = 本树；同一个核）：十份对拍语料各十个报告面，89/100 逐字节相同；不同的 11 面都在预期里——html 九面（HTML 成了判决语言）、java 与 r 的 docdup 各一面（骨架表多了 Javadoc / roxygen 标签：java 过门的段 4 → 3、r 送判的对 10 → 6，两边的重复都是 0）。七个负向探针（走查范围、产物规则、名字域、源集可见性、本单元、本源根、Lua 本目录）各自转红，源码还原逐字节相等。
- **查重**：子仓本批自己的测试带进六块新克隆（125 > 预算 119），全部在构造上消掉、预算未动——生成器三段方法说明挪到各自的生成器旁、两个篡改门共用 `bumped`、Java 与 Lua 的阶梯电池各并回一段文字（`common::text_ladder`「一语言一段」的规则）、java_sets 的两张表并成一张、HTML 站点对表按列读、HTML 的两道提及题挪到 Markdown 标题旁。
- **文档**：README 双语的语言行、技术栈图四张（判决语言补 C / C++ / Java / Lua / R / HTML——自步 2 起就漏了）、架构图 IR「十二套文法」重渲、方法学册 01 / 03 / 06 / 09 与方法学索引 03 行、官网工作原理页中英两版 03 节（文档文本的列举补 HTML 文本、常量表 `DOCDUP_REV` 6）、`docs/EVAL-SET-M5-3.md`（其后两个基准冻结点的出处行号 88-95 → 94-101，`contracts/bench/bench.json` 改、三个生成面随之重生成）、`docs/EVAL-SET-LANGS.md`（HTML 考题、Java / Lua / R 退役待重判、篡改门的神谕档）、`docs/FPR-REPLAY.md`、`docs/RELEASE.md` §0（重放一条）、设计册 §13 与 §14 第 18–21 条、`cli/Cargo.toml` 文法注释、计划书横幅与 §6 T 轨步 5 + **步 5b 边界清零**（2026-09-26 用户裁「1.8.0 之前修完」：全仓普查 215 条写明的边界里能按规范修对的 32 + 3 条，排在步 6 评估之前）。
- 门：主 check 944 / dedup 59 / scan 85 warn 0 fail（deadcode 0 / docdup 0 / erase 0），子仓 982 / 119 / 42 warn 0 fail（本批长过 50 行的两个函数拆回线内：精度册核对的逐行部分拆成 `verify_row`，站点用例表改成常量；本批新写的 `product_walk` 只在本文件用，收回私有），两仓棘轮 pass；lib 405 / it 455 (16 ign)——提交前的全量跑 426 绿 13 红，逐条归因：三条 `never committed` 是新文件未提交的提交序产物（`graph/ladder/java_sets.rs` 与 HTML 考题的样本），由提交后的 CI 执行；三条是本批漏改的面（冻结基准点引文的行号随 `EVAL-SET-M5-3.md` 移动、how 页 `DOCDUP_REV` 芯片、断言 GUI 页有正文段而它的标签全在收录下限以下），修后单跑绿；七条守卫 / daemon 腿在满载下连不上 daemon（守卫按设计放行、判决 JSON 为空），单跑 7/7 绿；核与 GUI 未动（`cabal test` 免跑），clippy（cli 全目标、gui `--locked`）+ fmt 清。ADR-006 具名重立两仓——主 `cli/src/graph/wire.rs` 119 → 132、`cli/src/graph/ladder/lua.rs` 108 → 133、`cli/src/graph/ladder/java.rs` 268 → 299、`cli/src/scan/walk.rs` 361 → 375、`cli/src/fourclass/units.rs` 231 → 261、`docs/EVAL-SET-LANGS.md` 382 → 413、`CHANGELOG.md` 692 → 708 超容差，五个新文件入列（`scan/html.rs`、`scan/outputs.rs`、`graph/sites/html.rs`、`graph/ladder/java_sets.rs`、`docdup/html.rs`），另十九个文件容差内同定，softLine 363 → 360；子 `it/eval_lang_parts/` 的 `mod.rs` 235 → 266、`precision.rs` 126 → 187、`score.rs` 231 → 250、`tamper.rs` 47 → 137、`generate.rs` 239 → 287，`it/eval_lang_precision.rs` 145 → 199、`it/lang_provenance.rs` 91 → 146、`it/eval_docdup_universe.rs` 56 → 91、`it/graph_ladder_java.rs` 94 → 126、`it/graph_ladder_lua.rs` 84 → 127、`unit/fourclass/units.rs` 108 → 132、`unit/docdup/exempt.rs` 103 → 122、`unit/docdup/segments_tests.rs` 122 → 187、`unit/graph/sites_tests.rs` 141 → 153 超容差，六个新文件入列（`it/eval_lang_parts/walk.rs`、`it/eval_lang_parts/replay.rs`、`it/walk_outputs.rs`、`unit/scan/outputs.rs`、`unit/graph/sites/html.rs`、`unit/graph/ladder/java_sets.rs`），另七个文件容差内同定，softLine 295 → 303。

**无默认档位变更。** 语言扩展 v2.30 步 5 HTML 提交 A″：盲评真值冻结（2026-09-26；只加冻结工件与它的门，判决代码字节零变化）：

- **真值表** `contracts/eval/lang-review-codeeraser-v1.json`（37 行）、`lang-review-learning-area-v1.json`（63 行）与 `lang-review-html5-boilerplate-v1.json`（0 行——它的 6 个站点没有一个被秩选中，表为冻结集的完整性立档）：四个独立 Opus 代理各判一批 25 道主样本（按审阅序切批），只读两个语料在钉住 tip 的干净克隆和自己那一批，没看过产品的任何解析结果、没跑过 ce；装配逐字照录，判决不动。100 道的 spec 全在记录的行上，零失配，没有动用备用题。真值 = 站点所引 URL 在该语料**部署出来的站点**上服务的文件（部署从仓内证据读出：本仓 = Cloudflare Pages 以 `site/` 为源根，learning-area = GitHub Pages 项目站 `/learning-area/` 一比一映射仓根）：external 21、文件 71（其中宇宙内的页面 14、宇宙外的样式 / 图片 / 脚本 / 媒体 57）、节 6、`dynamic` 1（Flask 模板 `{{ url_for('hello') }}`）、`none` 1（该目录下没有的 `bear.mp3`）。简报把 `nth` 写成了同类序位（样本按该行全部站点计），发现后四个代理各自重查 25 行，只有一行改判（`site/zh/how/index.html:31` 的 `img`，mismatch → `site/icon-256.png`），经过记在 `docs/EVAL-SET-LANGS.md` HTML「真值」一节与表的 `auditor` 字段。候选漏检：codeeraser 42 条全是 `og:url` / `og:image` 的 meta `content`，learning-area 8 条是 `<style>` 里的 `url()` 与内联 `<script>` 里的 URL 字符串，`scope_gaps` 5 条在 CSS / JS / Python 文件上——都在 HTML 属性站点表之外，判分时逐条附检测器在那一行读到的站点。
- **钉住的树**（新冻结族 `contracts/eval/lang-tree-{codeeraser,html5-boilerplate,learning-area}-v1.json`，757 / 80 / 594 个路径）：HTML 的真值够得到钉住树里任何被追踪文件（页面取的是站点服务的东西），而审阅门没有克隆——树档 = `git ls-tree` 在 tip 列出的每个路径，只是 tip 的函数；门把它对着宇宙核（宇宙文件全在树上，其余按宇宙自己的排除计数逐类对上），审阅表的真值与 `scope_gaps` 改绑到「这门考题的真值够得到的文件」（考题表新字段 `reach`：Java / Lua / R 绑宇宙，HTML 绑树），精度册生成器有克隆时把树重导一遍。
- **门**（子仓）：`it/eval_lang_parts/tree.rs`（生成器 `--ignored lang_tree` + 核对 `verify_tree`）、`it/eval_lang.rs` 两腿（每份树档核对；删一个宇宙文件 / 塞一页 / 调换两行 / 改方法句 / 换 tip 五种篡改各拒）、`it/eval_lang_review.rs` 新腿 `a_truth_beyond_the_tree_is_refused`（宇宙外树内的真值通过、树外的拒），「包目录真值」探针改读核对器自己的分类（一条图片路径此前会被当成包目录，让通用腿把 HTML 表也当成含包真值的表）；考题表的 `audited` / `scored` 两个布尔旗合成有序的 `Stage`（sampled → audited → scored，「已判分 ⇒ 已审阅」由构造保证；也正是它让形状相同的考题行回到查重门的窗口之下——每行 55 记号 → 49），HTML 翻为 audited；`Exam::assert_envelope` 收拢精度册与树档共用的信封核对（查重守卫在写树档时拦下的那一块）。顺序门：审阅表提交后 HTML 的阶梯才可落地；本提交不碰 `cli/src`。
- 门：主 check 944 / dedup 59 / scan 85 warn 0 fail（deadcode 0 / docdup 0 / erase 0），子仓 982 / 119 / 42 warn 0 fail（deadcode 0 / docdup 0 / erase 0；`it/eval_lang_parts/mod.rs` 一度 303 行过 300 软线、子仓 scan 42 → 43 warn，三处文档注释回折到段宽、`source_row` 的 json 行收成 `sample_constants` 已有的紧凑形，回到 299 行、42 warn），两仓棘轮 pass；lib 405（`cli/src` 本提交零改动，复跑全绿）/ it 460 (17 ign；新增五条 = `lang_trees_verify`、`a_tampered_tree_is_refused`、`stages_are_filed_in_order`、`a_truth_beyond_the_tree_is_refused` 与 `--ignored` 的树档生成器 `lang_tree`)——提交前的全量跑 441 绿 2 红：`lang_provenance` 两条对尚未提交的审阅表 `lang-review-codeeraser-v1.json` 报 `never committed`，与步 4 的 A′ 同是提交序的产物，提交后 CI 的全量是执行者；clippy（cli 全目标）+ fmt 清；判决代码与核未动，`cabal test` 不涉及。ADR-006 具名重立两仓——主 `docs/EVAL-SET-LANGS.md` 413 → 475 超容差（cap 423，长出来的是仪器一节的「钉住的树」一条与 HTML「真值」一节），`CHANGELOG.md` 708 → 715 容差内同定，softLine 360 不动；子 `it/eval_lang_parts/mod.rs` 266 → 299（cap 276）、`it/eval_lang.rs` 148 → 216（cap 158）、`it/eval_lang_review.rs` 169 → 210（cap 179）超容差，新文件 `it/eval_lang_parts/tree.rs` 128 入列，`walk.rs` 82 → 91、`review.rs` 143 → 152、`eval_lang_precision.rs` 199 → 203、`generate.rs` 287 → 288 容差内同定，softLine 303 不动；重立后 check 944（size 轴 84 → 86）；六份档与子仓的新文件入普查宇宙，册 13 自仓行 U 1169 → 1176。
