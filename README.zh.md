...
5. **揪出没有人到达的东西。** 文件级引用图承载四态存活性判决与环成员：死代码与死文档是被点名的，不是猜出来的。判决之侧另有符号层顾问：列出树中没有任何他文件拼写其名的声明——带可见性码报告，永不判决。
...
所有的线，基线记录它**在哪份 `ce.toml` 下立的指纹**（规范化的有效旋钮集：与出厂默认不同的值——注释、键序、写成默认值的旋钮、没人声明过的可选项都不动它）：改 glob、改阈值、改分数旋钮、改 exclude——凡是解析结果
里有且改变判决的，`ce check` 都会**具名停下**（`knobs_digest`），而不是悄悄放松。同意一份新配置是它自己的具名动作——
...
配置等于出厂默认的仓库判决逐字节不变、不发指纹、基线文件也不多一个键，把默认值写出来的仓库亦然；一旦声明类，文件被量的线就变了，
跨这道开关的分数**不可比较**。
...
以自己的 `ce.toml` 与基线过同一套六道门。crate 的单元测试也住在那里（`unit/`，每个
`#[cfg(test)]` 模块一个文件，由源文件以 `#[path]` 挂回），本仓分数只量产品代码。
