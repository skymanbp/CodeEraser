...
malformedNode :: Bool
malformedNode = case respond "5.1.0" req of
  Left (_, code, msg) ->
...
deadOf req = do
  Right bytes <- pure (respond "5.1.0" req)
...
unresRefused =
 ...
 where
  refusedMsg unres want = case respond "5.1.0" (graphReq unres) of
...
emptyIsAbsent :: Bool
emptyIsAbsent = respond "5.1.0" (symReq (Just [])) == respond "5.1.0" (symReq Nothing)
...
symRefused =
 ...
 where
  refusedSym syms want = case respond "5.1.0" (symReq (Just syms)) of
    Left (_, code, msg) -> code == "contract" && msg == want
    Right _ -> False
