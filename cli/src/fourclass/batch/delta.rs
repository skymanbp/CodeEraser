...
/// Apply the delta. A returned line MUST be one of the sent leftovers
/// AND not yet consumed — the leftover check alone accepted the same
/// line twice, and the second decrement underflowed the class count
/// (usize wrap: ~18e18 "deleted" lines) instead of failing loudly
/// (review 2026-08-19, codex lane). Consuming from a per-side set
/// makes both violations the same named error.
pub(super) fn merge(
    reply: &Value,
    inputs: &[PairInput],
    sent: &[(Side, Side)],
...
mod tests {
    ...
    /// "deleted" lines in release).
    #[test]
    fn a_double_listed_delta_line_is_refused_not_underflowed() {
        let (inputs, sent, mut pairs) = one_leftover();
        let reply = json!({"moved": [[0, [1, 1], []]], "blocks": []});
        let err = merge(&reply, &inputs, &sent, &mut pairs).expect_err("double apply");
    ...
    fn a_single_delta_line_reclassifies_once() {
        ...
        merge(&reply, &inputs, &sent, &mut pairs).expect("single apply");
        assert_eq!(pairs[0].counts.removed_deleted, 0);
        assert_eq!(pairs[0].counts.removed_moved, 1);
    }
}
