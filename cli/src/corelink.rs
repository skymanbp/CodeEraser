...
/// first [[rules.class]] glob match, 0 = none) — one arity per
/// table, mixed refuses — beside the additive `classKnobs` table
/// [[classId, code, value]] whose codes are the ceilings' own 0/1/2
/// (sizeCeil / cocCeil / sizeHard) under a class; the reply echoes
/// the table exactly when it rode. Names and globs never cross; the
/// ratchet never sees the column and the baseline stays three
/// columns. An unclassed repo's bytes are unchanged.
...
/// constant rewrite + append and the 5th a survived count nothing
/// ever measured, and the core read neither. A removed shape is a
/// breaking change (§2), so the major moves; every golden's request
/// line says 3.0.0 and its reply was regenerated by the core.
