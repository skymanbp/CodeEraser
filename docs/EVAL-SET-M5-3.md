...
precision 32 行普查零漂移、只有计数回声与 `docdup_rev` 变）；rev 5→6 = HTML 文本成第四种段 `html_text`、
骨架前缀表加 Javadoc / Doxygen / roxygen / LDoc 标签并认 `--` 注释引导（2026-09-26，计划 v2.30 步 5）：
五份段宇宙的扩展名表是冻结的（go / md / py / rs / ts，无 html），逐文件字段只有 zod 一处移动——
