...
| Lua | `require "a.b"`→`a/b.lua`，再 `a/b/init.lua`，找遍每个搜索目录：仓根、`src`、`lua`、声明根，外加树里文件自己写进 `package.path` 的字面模板（`lua_path.rs` 在遍历时读，经 `Scope::lua` 交给阶梯，每个文件的模板入 resolve_key；只认字面字符串与 `..` 拼接里的字面段、相对路径、恰一个 `?`、以 `.lua` 结尾）。落码时加的模板一读：koreader 的 `frontend/` 只写在它自己的 `package.path` 里，只按默认根设计时它的 78 道真值只对 20；两个目录命中不同文件 = ambiguous_root（一次运行先试哪个模板取决于路径的拼法，不是文本事实） | `dofile`/`loadfile`：引用文件同目录，再仓根，取第一个 | — | 标准库与 LuaJIT 内建名（`string` `table` `os` `ffi` `jit`…，共 18 个）→External，**先于搜索**：`package.loaded` 先答，树里有同名文件也轮不到它；其余 OutOfScope（rock 或 C 库提供的模块、运行时算出来的目录，如 `string.format("%s/?.lua", dir)`） |
