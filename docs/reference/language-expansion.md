...
| HTML | 相对：文档目录（有 `<base href>` 则按它——相对 base 落在文档目录下，根相对 / 同主机绝对 base 走 R2/R3，别主机的 base 让相对引用全部 External）；目标 = 任何被走文件——页面、文档、代码、**资产**（走查读到而索引不持有的文件，`WalkIndex::assets`，进 resolve_key）；目录目标→`dir/index.html` 再 `index.htm`（Web 服务器语义，与 md「目录=包」不同，明说）；`?query` 去掉，`&amp;` 与百分号转义先解码 | `/abs`：先 `[graph.search_roots] html` 声明目录（两目录命中 = ambiguous_root），无声明则由文档自己的服务 URL 推出部署根（`html_head.rs`：canonical、`og:url`、本页语言的 hreflang alternate——`/how/` 服务 `site/how/index.html` ⇒ 根 `site`）；跨文件 `#frag`：目标页 `id` 集（零/多命中降级到文件级，md slug 先例；目标是 md 则问 md 的 slug 集） | 无声明也推不出根时：页面的每个祖先目录（部署根必在其中）恰一个持有该路径则答；两个 = ambiguous_root，零 = out_of_scope | 裸 `#frag`：本文件节声明如实取（R4，md 同读；`#` 与纯 `?query` = 本页）；任何 scheme、`//`、`mailto:`/`tel:`/`javascript:`/`data:`、别主机的绝对 URL→External（R5，md 同读）；本主机的绝对 URL = 它的根相对路径；空值：`href`/`action` = 本页（空 URL 即文档自身，URL Standard），`src`/`srcset`/`link_asset` = 台账行 `empty`（HTML 的 img / script / link 取回算法遇空值即返回，什么也不取） |

- 边种类：HTML `href`/`action` 到页面 = 链接边（存活）；`src`/`srcset`/`link rel=stylesheet|icon|preload` 到资产 = asset 边（核的 `assetKind` 惰性规则原样适用，目标是资产结点：`Node::asset`——走查读到而索引不持有的文件，`nodes.rs` 按构造标出〔阶梯的候选集 = 索引持有的被走文件 ∪ 不持有的被走文件，包目标带自己的粒度，索引不持有的文件结点只能是资产〕；结点行单独发角色位 9 `ROLE_ASSET = 1 << 9`，核 `roleBits` 第十行 `(9, 4)` 落在 dyn-referenced 位——样式表的 `url()`、脚本的取回、manifest 的图标是图看不见的引用，资产永不成候选；也不进被量层 `measured_nodes`，结构族、分数与 join 不放它——步 5 提交 B 在本仓实测：不标之前 27 个资产〔gui/ui 的脚本与样式、站点的图片与样式表〕被报成死文件，`ce structure` 在两份 HTML 语料上以「graph node … outside the walked tree」拒答）。`type_ref` 与 import 边落同一目标时由 BTreeSet 去重。
...
| 5 | HTML（进行中，四个提交）：提交 A（2026-09-26）：`Attr` 站点、节锚单元、docdup kind 3 与 shed 连同 §9 的骨架前缀表（`DOCDUP_REV` 6）、三语料考题冻结（本仓、h5bp、MDN learning-area）、三处根修（构建产物只在项目文件旁排除、Java 源集与本单元、Lua 引用文件自己的目录）；A″ 盲评真值（2026-09-26：100/100 在记录行上判出，真值可及整棵钉住树——`lang-tree-*` 档）；B 阶梯（2026-09-26：`html.rs` + `html_head.rs`——资产候选集 `WalkIndex::assets` 与每页的 `id` 集入 resolve_key、部署根由声明或页面自己的服务 URL 推出、祖先目录推断、空值按元素的取回算法分读；考题的走查记录加 `unreached`〔树内、宇宙外、走查拒读的路径，真值指向它按站外判分〕；自食：本仓 309 个 HTML 站点全部解出，`site/` 无孤页）；B′ 九份精度册随后在 B 之上重生成 | `site/` 八页孤页/断链为零或已豁免有据 |
