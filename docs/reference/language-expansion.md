...
- 走查表：`BUILTIN_EXCLUDES` 已含 `vendor/ build/ target/ dist/ node_modules/`，Java 的 `target/` 与 Gradle 的 `build/` 已被覆盖；新增 R 的 `renv/`、`packrat/` 与 luarocks 的项目树 `lua_modules/`（`luarocks init` 所建并写进 `.gitignore`，node_modules 的孪生；步 4 落码。`.luarocks/` 是隐藏目录，走查本就不进）。
...
| 4 | Lua + R（进行中，三个提交）：提交 A（2026-09-25）：表 `spec_lua.rs` / `spec_r.rs`、函数值命名 `scan/binding.rs`（Lua 的 `local` 可见起点进递归弧，成员形名的对象是表不是类）、调用站点表 `CallSite`（`require` / `load` / `source` / `library` 与 `pkg::` 运算符）、可见性、Protocol 与测试名、自提及（字符串、LDoc `@usage`、roxygen `@examples` / `@examplesIf`）、入口角色、走查排除 `lua_modules/` `renv/` `packrat/`、电池 / 对拍（lizard：Lua CC 22/22；R CC 34 条一致、9 条差全归因，R 的 reader 是这时发现的，D0 / D32）、考题 slice（Lua luarocks + koreader、R stringr + covid19model，§14 第 17 条）+ sample 冻结；A′ 盲评真值；B 阶梯与精度册 | 同上 |
