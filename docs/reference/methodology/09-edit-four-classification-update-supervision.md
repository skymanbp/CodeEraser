...
Each moved line is attributed to the innermost unit containing it — `owner` picks the minimum-span unit ([units.rs:222-227](../../../cli/src/fourclass/units.rs#L222)). Unit keys are `name/arity` for code ([units.rs:65-70](../../../cli/src/fourclass/units.rs#L65)) and heading text for Markdown ([units.rs:171-193](../../../cli/src/fourclass/units.rs#L171)). Non-function named units (Rust `const_item`, `static_item`, `struct_item`, `enum_item`, `trait_item`, `mod_item`; Python `class_definition`; TS/TSX class/interface/enum declarations; since plan v2.17 L round step 8 also Go `type_spec`/`type_alias` and Haskell `data_type`/`newtype`/`type_synomym`/`class`/`type_family`/`data_family` — the register is the symbols domain too, so a named type form must be a unit to carry a visibility word) are registered only for relocation reporting, not for the M1 function metrics ([kinds.rs:75-97](../../../cli/src/fourclass/kinds.rs#L75)). Rust `impl_item` gets an impl key `impl Foo` / `impl Advisor for Foo` ([units.rs:154-167](../../../cli/src/fourclass/units.rs#L154)) so that methods of different impls are not seen as top-level.
