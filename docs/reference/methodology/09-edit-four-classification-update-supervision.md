...
Each moved line is attributed to the innermost unit containing it — `owner` picks the minimum-span unit ([units.rs:187-192](../../../cli/src/fourclass/units.rs#L187)). Unit keys are `name/arity` for code ([units.rs:49-54](../../../cli/src/fourclass/units.rs#L49)) and heading text for Markdown ([units.rs:137-159](../../../cli/src/fourclass/units.rs#L137)). Non-function named units (Rust `const_item`, `static_item`, `struct_item`, `enum_item`, `trait_item`, `mod_item`; Python `class_definition`; TS/TSX class/interface/enum declarations; since plan v2.17 L round step 8 also Go `type_spec`/`type_alias` and Haskell `data_type`/`newtype`/`type_synomym`/`class`/`type_family`/`data_family` — the register is the symbols domain too, so a named type form must be a unit to carry a visibility word) are registered only for relocation reporting, not for the M1 function metrics ([kinds.rs:31-53](../../../cli/src/fourclass/kinds.rs#L31)). Rust `impl_item` gets an impl key `impl Foo` / `impl Advisor for Foo` ([units.rs:120-133](../../../cli/src/fourclass/units.rs#L120)) so that methods of different impls are not seen as top-level.
...
The sensitivity test pins the knob as live: `destFloor == 2` and `not (siteOpens 2 1)` ([Spec.hs:136-137](../../../core/test/Spec.hs#L136)), and perturbing the site cost moves the floor — `s ∈ {0,2,4,6}` ⇒ floor `{1,2,3,4}` ([Spec.hs:138-141](../../../core/test/Spec.hs#L138)).
...
- **Wire indices are bounds-checked**, not used as slice subscripts, in both the merge and the report — the report path runs inside the daemon, which has no `catch_unwind` ([delta.rs:100-102](../../../cli/src/fourclass/batch/delta.rs#L100), [session.rs:118-128](../../../cli/src/fourclass/session.rs#L118)).
