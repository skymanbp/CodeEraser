...
Each moved line is attributed to the innermost unit containing it — `owner` picks the minimum-span unit ([units.rs:184-189](../../../cli/src/fourclass/units.rs#L184)). Unit keys are `name/arity` for code ([units.rs:49-54](../../../cli/src/fourclass/units.rs#L49)) and heading text for Markdown ([units.rs:134-156](../../../cli/src/fourclass/units.rs#L134)). Non-function named units (Rust `const_item`, `static_item`, `struct_item`, `enum_item`, `trait_item`, `mod_item`; Python `class_definition`; TS/TSX class/interface/enum declarations; since plan v2.17 L round step 8 also Go `type_spec`/`type_alias` and Haskell `data_type`/`newtype`/`type_synomym`/`class`/`type_family`/`data_family` — the register is the symbols domain too, so a named type form must be a unit to carry a visibility word) are registered only for relocation reporting, not for the M1 function metrics ([kinds.rs:31-53](../../../cli/src/fourclass/kinds.rs#L31)). Rust `impl_item` gets a typed key `impl Foo` / `impl Advisor for Foo` ([kinds.rs:72-77](../../../cli/src/fourclass/kinds.rs#L72), [units.rs:115-130](../../../cli/src/fourclass/units.rs#L115)) so that methods of different impls are not seen as top-level.
