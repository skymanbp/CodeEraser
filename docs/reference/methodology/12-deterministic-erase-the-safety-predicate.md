...
([VERSIONING.md:307-308](../../../contracts/VERSIONING.md#L307)).
...
([VERSIONING.md:307-311](../../../contracts/VERSIONING.md#L307)). The client
...
([Cost.hs:5-13](../../../core/app/CE/Erase/Cost.hs#L5)). Each class below is
...
([Cost.hs:43-45](../../../core/app/CE/Erase/Cost.hs#L43)). Rust no longer mints
this row — since 2.32.0 the dead road ships class 3 (below) — and the per-language fold
survives only for class-2 twin rows
([gather.rs:79-99](../../../cli/src/erase/gather.rs#L79)).

**Predicate.** `judgeRow` accepts the row exactly when `langUnresolved == 0`;
the successful result is `(True, 0)`
([Cost.hs:43-45](../../../core/app/CE/Erase/Cost.hs#L43)).

**Guard.** Any non-zero unresolved count emits reason `1` and keeps the row
`False` ([Cost.hs:43-45](../../../core/app/CE/Erase/Cost.hs#L43)). Before that
...
raw-slice equality bit ([Cost.hs:46-49](../../../core/app/CE/Erase/Cost.hs#L46)).
...
([Cost.hs:47-48](../../../core/app/CE/Erase/Cost.hs#L47)). Passing both tests
returns `(True, 0)` ([Cost.hs:49](../../../core/app/CE/Erase/Cost.hs#L49)).

**Guard.** A short verbatim run refuses with reason `2`; a byte mismatch refuses
with reason `3` ([Cost.hs:47-48](../../../core/app/CE/Erase/Cost.hs#L47)). The
...
([Cost.hs:50-55](../../../core/app/CE/Erase/Cost.hs#L50)). Rust first finds a
...
([Cost.hs:50-55](../../../core/app/CE/Erase/Cost.hs#L50)).
...
([Cost.hs:50-55](../../../core/app/CE/Erase/Cost.hs#L50)). This is why a T1
...
The same candidate family as class 0 with the trust judgment moved to its owner: fact 1 is no longer a locally folded per-language unresolved count but the graph family's OWN per-row confidence (book 06 §8 — 0 unvouched / 1 vacuous / 2 vouched), and the predicate refuses only at 0 ([Cost.hs:56-58](../../../core/app/CE/Erase/Cost.hs#L56)). Shape: the dead verdict stays bounded 1..4 and the confidence 0..2 ([Erase.hs:34-37](../../../core/app/CE/Erase.hs#L34)). The Rust planner refuses a dead row that carries no confidence — a reply whose request never shipped the ledger licences nothing ([gather.rs:114](../../../cli/src/erase/gather.rs#L114)). Class 0 keeps judging unchanged for the grace window (the staleDocs discipline) and retires in a later minor; the class-2 twin row deliberately keeps its local count — a twin row is not a graph dead row, so no core confidence exists for it ([Cost.hs:16](../../../core/app/CE/Erase/Cost.hs#L16)).
...
|---|---|---|
| `0 eraseable` | every class-specific safety test passed | class 0 falls through after `langUnresolved == 0`, class 1 after full-segment and byte tests, or class 2 after all four checks ([Cost.hs:43-55](../../../core/app/CE/Erase/Cost.hs#L43)) |
| `1 language_unresolved` | the owning language still has unresolved graph sites | class 0: `langUnresolved /= 0`; class 2: the final guard sees `langUnresolved /= 0` ([Cost.hs:43-45](../../../core/app/CE/Erase/Cost.hs#L43), [Cost.hs:50-55](../../../core/app/CE/Erase/Cost.hs#L50)) |
| `2 not_full_segment` | the reported verbatim run does not cover either segment | class 1: `verbatim < wordsA || verbatim < wordsB` ([Cost.hs:46-47](../../../core/app/CE/Erase/Cost.hs#L46)) |
| `3 bytes_differ` | the candidate and its survivor are not byte-identical | class 1 or class 2: `bytesEqual /= 1` ([Cost.hs:46-52](../../../core/app/CE/Erase/Cost.hs#L46)) |
| `4 copy_not_dead` | the T1 twin's target file is not graph-dead | class 2: `copyFileDead /= 1`; the catch-all malformed-row arm also refuses with this code ([Cost.hs:50-59](../../../core/app/CE/Erase/Cost.hs#L50)) |
| `5 unit_not_covered` | the duplicate block does not cover a whole unit | class 2: `unitCovered /= 1` ([Cost.hs:50-51](../../../core/app/CE/Erase/Cost.hs#L50)) |
...
([Cost.hs:39-41](../../../core/app/CE/Erase/Cost.hs#L39)). A row whose shape does
not match any class is refused by the final catch-all rather than reaching an
eraseable result ([Cost.hs:59](../../../core/app/CE/Erase/Cost.hs#L59)).

### 4. Capacity and degraded replies

`eraseRowCap` is `4096` ([Cost.hs:33-37](../../../core/app/CE/Erase/Cost.hs#L33)).
...
([VERSIONING.md:316-318](../../../contracts/VERSIONING.md#L316)).
...
rows as `error/contract` ([VERSIONING.md:312](../../../contracts/VERSIONING.md#L312)).
...
([Cost.hs:1-5](../../../core/app/CE/Erase/Cost.hs#L1), [Cost.hs:39-42](../../../core/app/CE/Erase/Cost.hs#L39)).
...
not an additional rule ([Cost.hs:43-58](../../../core/app/CE/Erase/Cost.hs#L43),
[VERSIONING.md:304-318](../../../contracts/VERSIONING.md#L304)). No other
