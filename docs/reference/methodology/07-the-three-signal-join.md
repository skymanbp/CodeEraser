...
- **The lattice (Haskell, pure).** `CE.Verdict.Join` takes `Knobs -> Legs` and returns `(verdict, legsMask, reasonBits)` ([Join.hs:147-149](../../../core/app/CE/Verdict/Join.hs#L147)). It is reached over the `verdict/1` wire by `ce check`, which builds one `candidates` row per sim row ([Candidates.hs:17-27](../../../core/app/CE/Verdict/Candidates.hs#L17)).
...
One measurement caveat: today's only producer of sim rows emits `[u, v, 0, 100, 100]` — kind `t1t2` at ratio `100/100`, because the pair reached the table only by carrying a verified block ([score/mod.rs:219](../../../cli/src/score/mod.rs#L219), fn doc at [score/mod.rs:199-201](../../../cli/src/score/mod.rs#L199)). So on the live `ce check` road the similarity bit holds for every candidate row by construction; kinds `1` and `2` are exercised only by the lattice's own battery ([JoinProps.hs:49-50](../../../core/test/JoinProps.hs#L49), [JoinProps.hs:65-66](../../../core/test/JoinProps.hs#L65)).
...
Per side the leg is `(appended, rewrote)` line counts over the window ([Join.hs:62-63](../../../core/app/CE/Verdict/Join.hs#L62), [churn_unit.rs:33-37](../../../cli/src/join/churn_unit.rs#L33)). At tier F they are summed from the per-unit ledger so the report totals and the join legs come from one bookkeeping ([mod.rs:167-177](../../../cli/src/join/mod.rs#L167)); the wire row is `[u, rewrote, appended]` — three columns since proto 3.0.0, when the constant fourth (`rewrote + appended`) and the never-measured fifth (`survived`, always 0) were cut ([score/mod.rs:293-296](../../../cli/src/score/mod.rs#L293)), decoded back as `(appended, rewrote)` at [Candidates.hs:37](../../../core/app/CE/Verdict/Candidates.hs#L37). A pair's co-change count is a separate table, `[u, v, count]` ([score/mod.rs:319-325](../../../cli/src/score/mod.rs#L319)).
...
([Join.hs:180-182](../../../core/app/CE/Verdict/Join.hs#L180)). Making the order data is what lets the battery falsify it: the `reorder` probe judges a crafted row with a rotated table and requires the answer to flip from merge to `3` ([JoinProps.hs:24](../../../core/test/JoinProps.hs#L24), [JoinProps.hs:201-207](../../../core/test/JoinProps.hs#L201)).
