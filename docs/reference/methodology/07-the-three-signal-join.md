...
One measurement caveat: today's only producer of sim rows emits `[u, v, 0, 100, 100]` — kind `t1t2` at ratio `100/100`, because the pair reached the table only by carrying a verified block ([score/mod.rs:227](../../../cli/src/score/mod.rs#L227), fn doc at [score/mod.rs:207-209](../../../cli/src/score/mod.rs#L207)). So on both live roads — `ce check` and, since 2.33.0, `ce join`, which calls the same `score::sim_rows` — the similarity bit holds for every candidate row by construction; kinds `1` and `2` are exercised only by the lattice's own battery ([JoinProps.hs:49-50](../../../core/test/JoinProps.hs#L49), [JoinProps.hs:65-66](../../../core/test/JoinProps.hs#L65)).
