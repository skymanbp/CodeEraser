...
- **The lattice (Haskell, pure).** `CE.Verdict.Join` takes `Knobs -> Legs` and returns `(verdict, legsMask, reasonBits)` ([Join.hs:147-149](../../../core/app/CE/Verdict/Join.hs#L147)). It is reached over the `verdict/1` wire by `ce check`, which builds one `candidates` row per sim row ([Verdict.hs:204-214](../../../core/app/CE/Verdict.hs#L204)).
...
One measurement caveat: today's only producer of sim rows emits `[u, v, 0, 100, 100]` — kind `t1t2` at ratio `100/100`, because the pair reached the table only by carrying a verified block ([score/mod.rs:218](../../../cli/src/score/mod.rs#L218), fn doc at [score/mod.rs:198-200](../../../cli/src/score/mod.rs#L198)). So on the live `ce check` road the similarity bit holds for every candidate row by construction; kinds `1` and `2` are exercised only by the lattice's own battery ([JoinProps.hs:49-50](../../../core/test/JoinProps.hs#L49), [JoinProps.hs:65-66](../../../core/test/JoinProps.hs#L65)).
...
Per side the leg is `(appended, rewrote)` line counts over the window ([Join.hs:62-63](../../../core/app/CE/Verdict/Join.hs#L62), [churn_unit.rs:33-37](../../../cli/src/join/churn_unit.rs#L33)). At tier F they are summed from the per-unit ledger so the report totals and the join legs come from one bookkeeping ([mod.rs:167-177](../../../cli/src/join/mod.rs#L167)); the wire row is `[u, rewrote, appended]` — three columns since proto 3.0.0, when the constant fourth (`rewrote + appended`) and the never-measured fifth (`survived`, always 0) were cut ([score/mod.rs:292-295](../../../cli/src/score/mod.rs#L292)), decoded back as `(appended, rewrote)` at [Verdict.hs:224](../../../core/app/CE/Verdict.hs#L224). A pair's co-change count is a separate table, `[u, v, count]` ([score/mod.rs:318-324](../../../cli/src/score/mod.rs#L318)).
...
- Each candidate is the 6-tuple `[u, v, code, reasonBits, legsMask, confidence]` (2.33.0), one per sim row ([Verdict.hs:210-211](../../../core/app/CE/Verdict.hs#L210)), typed Rust-side as `Vec<[i64; 6]>` ([wire.rs:87](../../../cli/src/score/wire.rs#L87)). The **confidence** is the leg-agreement count — of the legs present, how many contributed at least one held condition, judged through the attribution table `legBits` (sim = bit 1, graph = bits 2..6, churn = bits 7..8; [Join.hs:136](../../../core/app/CE/Verdict/Join.hs#L136), [Join.hs:142](../../../core/app/CE/Verdict/Join.hs#L142)) and pinned by the two-leg/three-leg probes ([JoinProps.hs:192](../../../core/test/JoinProps.hs#L192)). The **severity** column of the verdict table (delete 3 > merge 2 > hotspot 1 — data the battery pins beside the permutable table, [JoinProps.hs:186](../../../core/test/JoinProps.hs#L186)) ships once per reply as `joinSeverity` ([Verdict.hs:92](../../../core/app/CE/Verdict.hs#L92)); the report ranks with the core's numbers, never its own.
- The fail bit is a disjunction over four *named* conditions — `ratchet_over`, `discrete_added`, `floor`, `dedup_budget` ([Verdict.hs:179-185](../../../core/app/CE/Verdict.hs#L179), folded at [Verdict.hs:153-154](../../../core/app/CE/Verdict.hs#L153)). No verdict code appears in that list.
