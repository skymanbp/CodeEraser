...
- **Leg assembly (Rust, report only).** `ce join` aggregates the three legs into file-tier and unit-tier rows and prints them. Its module header states the stance outright: "REPORT-ONLY: the verdict lattice ... exists and is battery-tested, but its wire hookup is 3i; nothing here thresholds or gates" ([mod.rs:6-7](../../../cli/src/join/mod.rs#L6)). Output schema `ce.join-report/0.1.0` ([mod.rs:26](../../../cli/src/join/mod.rs#L26)).
...
([mod.rs:101-106](../../../cli/src/join/mod.rs#L101)). Clone blocks are folded into that key as `(block count, token sum)` ([mod.rs:107-110](../../../cli/src/join/mod.rs#L107)).
...
([Join.hs:148-152](../../../core/app/CE/Verdict/Join.hs#L148)), with `rewriteNum = 50` [Verdict/Cost.hs:81](../../../core/app/CE/Verdict/Cost.hs#L81), `rewriteDen = 100` [Verdict/Cost.hs:84](../../../core/app/CE/Verdict/Cost.hs#L84), `cochangeFloor = 2` [Verdict/Cost.hs:73](../../../core/app/CE/Verdict/Cost.hs#L73). Both are configurable per request: `rewriteNum`/`rewriteDen` are thresholds codes 1/2 and `cochangeFloor` is code 3, all echoed back in the effective-knob table ([Knobs.hs:69-75](../../../core/app/CE/Verdict/Knobs.hs#L69), [Knobs.hs:51-52](../../../core/app/CE/Verdict/Knobs.hs#L51), [Knobs.hs:95](../../../core/app/CE/Verdict/Knobs.hs#L95)).

`cochangeFloor = 2` is not an independent choice — it is the churn report's own table floor. Rust admits a pair into the co-change table only at `count >= 2`, then keeps the top 20 — a bare `pairs.truncate(20)` ([churn/mod.rs:209-218](../../../cli/src/churn/mod.rs#L209), truncate at [churn/mod.rs:216](../../../cli/src/churn/mod.rs#L216)); the numerically-coincident `COCHANGE_FILE_CAP = 20` ([churn/mod.rs:54](../../../cli/src/churn/mod.rs#L54)) is a different guard, skipping pair-counting for commits that touch more files than it, so the lattice can never claim heat the report would not even list ([Verdict/Cost.hs:69-73](../../../core/app/CE/Verdict/Cost.hs#L69)). Correspondingly `cochange` is `Option`: `None` means the pair sits below that floor — unknown-small, never a fabricated zero ([mod.rs:45-47](../../../cli/src/join/mod.rs#L45), [Join.hs:51-53](../../../core/app/CE/Verdict/Join.hs#L51)), and `maybe False (>= floor)` makes an unknown never fire ([Join.hs:152](../../../core/app/CE/Verdict/Join.hs#L152)). Churn *zeros*, by contrast, are real zeros: an absent ledger row means the unit genuinely saw no window edits ([churn_unit.rs:110-112](../../../cli/src/join/churn_unit.rs#L110), default at [churn_unit.rs:125-130](../../../cli/src/join/churn_unit.rs#L125)).
...
- `ce join` never calls the lattice at all — it assembles legs and prints them ([mod.rs:59-74](../../../cli/src/join/mod.rs#L59)) — and its command runs through `family_cmd`, which passes the no-veto `|_| None` closure into `emit_checked` and therefore always exits `SUCCESS` ([main_judge.rs:174-181](../../../cli/src/main_judge.rs#L174), [main_judge.rs:104-109](../../../cli/src/main_judge.rs#L104), [main_judge.rs:202-213](../../../cli/src/main_judge.rs#L202)). Its console tail says so in the summary line: `"report-only; verdict lattice lands in 3i"` ([mod.rs:228](../../../cli/src/join/mod.rs#L228)).
- Degradation is visible, not silent: a degraded graph reply's reason rides the report ([mod.rs:64](../../../cli/src/join/mod.rs#L64)) and prints as `"join graph leg degraded: {}"` ([mod.rs:219](../../../cli/src/join/mod.rs#L219)). On the scoring road a degraded graph reply is refused outright rather than scored on an empty `pos` table ([score/mod.rs:168-180](../../../cli/src/score/mod.rs#L168)).
