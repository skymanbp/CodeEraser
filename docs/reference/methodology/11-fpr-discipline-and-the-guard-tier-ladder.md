...
Since the K-round root fix the duplicate-write rule judges **novel** duplication only: the probe's matches are subtracted by the matches the REPLACED content already carried — Write replaces the on-disk file, Edit replaces `old_string` — so a full rewrite of a file carrying budgeted blocks, or an edit inside one, is carried duplication and stays silent, while an introduction still denies ([guard.rs:170-200](../../../cli/src/guard.rs#L170)). A split to a NEW file has no replaced content and still denies; its reason teaches the ordering that passes — trim the source first, because candidate verification reads the current tree ([say.rs:26-28](../../../cli/src/guard/say.rs#L26)). The baseline probe runs only when the first probe matched, and a degraded baseline subtracts nothing: allowance never rides an unanswered question. Every sentence the two rule classes speak lives in one module, English and Chinese side by side ([say.rs:1-19](../../../cli/src/guard/say.rs#L1)), because they did not: the templates sat inline at six call sites across two files, nothing asked them as a set, and this face answered `CE_LANG=zh` in English while the Stop audit had answered in Chinese since M8-G3b.

Determinism is bought by *replaying the write* rather than estimating it. `resulting_lines` computes the exact post-write line count — `Write` is its own payload (`content.lines().count()`, [budget.rs:152-155](../../../cli/src/guard/budget.rs#L152)); `Edit` is the payload applied to the on-disk file under Edit's own semantics: CRLF-normalized, and a non-`replace_all` edit requires a **unique** match, matching the real tool's rejection of ambiguity ([budget.rs:132-147](../../../cli/src/guard/budget.rs#L132)). Any case where the tool call would fail on its own — missing file, empty or absent `old_string`, ambiguous match — returns `None` and the rule stays silent ([budget.rs:152-172](../../../cli/src/guard/budget.rs#L152)). The gate never judges a write that will not land.
...
- The hook is **fail-open**: any internal failure allows the edit, and the degraded run lands in the observe feed ([guard.rs:5-6](../../../cli/src/guard.rs#L5)); a probe that cannot reach the daemon returns `None`, which is logged as `degraded` rather than treated as "no duplicates" ([guard.rs:214-232](../../../cli/src/guard.rs#L214), [guard.rs:291](../../../cli/src/guard.rs#L291)).
...
A mistyped mode falls through the same `match` to no output ([guard.rs:258](../../../cli/src/guard.rs#L258)) — it can never enforce — while the degraded string still rides the observe feed's `mode` field ([guard.rs:276](../../../cli/src/guard.rs#L276), [guard.rs:282-296](../../../cli/src/guard.rs#L282)) and the health line.
...
When several rules fire on one write, the decision line is emitted at the **strongest** tier among them, ranked by index in `TIERS` (unknown values rank 0 = `observe`): class rules carry the class mode, the zone rule carries its own mapped tier, so a zone warn never rides a deny-class escalator ([guard.rs:126-143](../../../cli/src/guard.rs#L126)). A broken `ce.toml` overrides the computed tier down to a visible `warn` that names the parse error ([guard.rs:152-156](../../../cli/src/guard.rs#L152)).
...
([budget.rs:191-196](../../../cli/src/guard/budget.rs#L191); `S` is read off the committed `ce-baseline.json` via `committed_soft`, [budget.rs:258-263](../../../cli/src/guard/budget.rs#L258); `file_lines_warn = 300`, [ce-toml.md:17](../ce-toml.md#L17)). A degenerate zone — `H == 0` (no hard line), `H <= S`, or `lines <= S` — logs nothing rather than fabricating a position ([budget.rs:193-195](../../../cli/src/guard/budget.rs#L193)).

The map is a three-way partition on that per-mille ([budget.rs:231-240](../../../cli/src/guard/budget.rs#L231)):
...
The switch is **off by default** and changes one repo only: `[guard] zone_tiers`, default `false` ([tier.rs:24-28](../../../cli/src/config/tier.rs#L24), [ce-toml.md:32](../ce-toml.md#L32)). Disarmed, the zone is feed-only — a `zone` line is written in every tier and `zone_assess` returns `None`, injecting nothing ([budget.rs:213-219](../../../cli/src/guard/budget.rs#L213)). Armed, the resolved tier is additionally stamped as `zone_tier` on the feed line, so the record says what the rule *did*, not only where the write landed ([budget.rs:213-215](../../../cli/src/guard/budget.rs#L213), [hookio.rs:41-44](../../../cli/src/hookio.rs#L41)).

The default stays `observe` for one reason, written at the declaration site: §4.2's FPR discipline gates any default flip on the zone feed's own record ([tier.rs:24-28](../../../cli/src/config/tier.rs#L24)), and the `zone` event exists precisely as "the per-rule record any future zone→tier promotion must argue its FPR case from" ([hookio.rs:46-49](../../../cli/src/hookio.rs#L46)). The rule class has no FPR ledger yet, therefore it does not enforce by default — the same admission test the two promoted classes had to pass.
