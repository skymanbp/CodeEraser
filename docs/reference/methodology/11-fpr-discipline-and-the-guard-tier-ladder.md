...
The switch is **off by default** and changes one repo only: `[guard] zone_tiers`, default `false` ([tier.rs:24-28](../../../cli/src/config/tier.rs#L24), [ce-toml.md:32](../ce-toml.md#L32)). Disarmed, the zone is feed-only — a `zone` line is written in every tier and `zone_assess` returns `None`, injecting nothing ([budget.rs:207-213](../../../cli/src/guard/budget.rs#L207)). Armed, the resolved tier is additionally stamped as `zone_tier` on the feed line, so the record says what the rule *did*, not only where the write landed ([budget.rs:207-209](../../../cli/src/guard/budget.rs#L207), [hookio.rs:31-34](../../../cli/src/hookio.rs#L31)).
