...
Since the K-round root fix the duplicate-write rule judges **novel** duplication only: the probe's matches are subtracted by the matches the REPLACED content already carried — Write replaces the on-disk file, Edit replaces `old_string` — so a full rewrite of a file carrying budgeted blocks, or an edit inside one, is carried duplication and stays silent, while an introduction still denies ([guard.rs:168-198](../../../cli/src/guard.rs#L168)). A split to a NEW file has no replaced content and still denies; its reason teaches the ordering that passes — trim the source first, because candidate verification reads the current tree ([say.rs:26-28](../../../cli/src/guard/say.rs#L26)). The baseline probe runs only when the first probe matched, and a degraded baseline subtracts nothing: allowance never rides an unanswered question. Every sentence the two rule classes speak lives in one module, English and Chinese side by side ([say.rs:1-19](../../../cli/src/guard/say.rs#L1)), because they did not: the templates sat inline at six call sites across two files, nothing asked them as a set, and this face answered `CE_LANG=zh` in English while the Stop audit had answered in Chinese since M8-G3b.

Determinism is bought by *replaying the write* rather than estimating it. `resulting_lines` computes the exact post-write line count — `Write` is its own payload (`content.lines().count()`, [budget.rs:126-129](../../../cli/src/guard/budget.rs#L126)); `Edit` is the payload applied to the on-disk file under Edit's own semantics: CRLF-normalized, and a non-`replace_all` edit requires a **unique** match, matching the real tool's rejection of ambiguity ([budget.rs:131-146](../../../cli/src/guard/budget.rs#L131)). Any case where the tool call would fail on its own — missing file, empty or absent `old_string`, ambiguous match — returns `None` and the rule stays silent ([budget.rs:126-146](../../../cli/src/guard/budget.rs#L126)). The gate never judges a write that will not land.
...
- The hook is **fail-open**: any internal failure allows the edit, and the degraded run lands in the observe feed ([guard.rs:5-6](../../../cli/src/guard.rs#L5)); a probe that cannot reach the daemon returns `None`, which is logged as `degraded` rather than treated as "no duplicates" ([guard.rs:212-230](../../../cli/src/guard.rs#L212), [guard.rs:289](../../../cli/src/guard.rs#L289)).

### The tier ladder

Four tiers, and nothing else is a tier: `TIERS = ["observe", "warn", "ask", "deny"]` ([tier.rs:40](../../../cli/src/config/tier.rs#L40)). They map onto Claude Code's decision JSON at one emission point ([guard.rs:249-266](../../../cli/src/guard.rs#L249)):
...
A mistyped mode falls through the same `match` to no output ([guard.rs:256](../../../cli/src/guard.rs#L256)) — it can never enforce — while the degraded string still rides the observe feed's `mode` field ([guard.rs:274](../../../cli/src/guard.rs#L274), [guard.rs:280-294](../../../cli/src/guard.rs#L280)) and the health line.
...
When several rules fire on one write, the decision line is emitted at the **strongest** tier among them, ranked by index in `TIERS` (unknown values rank 0 = `observe`): class rules carry the class mode, the zone rule carries its own mapped tier, so a zone warn never rides a deny-class escalator ([guard.rs:124-141](../../../cli/src/guard.rs#L124)). A broken `ce.toml` overrides the computed tier down to a visible `warn` that names the parse error ([guard.rs:150-154](../../../cli/src/guard.rs#L150)).
...
([budget.rs:185-190](../../../cli/src/guard/budget.rs#L185); `S` is read off the committed `ce-baseline.json` via `committed_soft`, [budget.rs:252-257](../../../cli/src/guard/budget.rs#L252); `file_lines_warn = 300`, [ce-toml.md:17](../ce-toml.md#L17)). A degenerate zone — `H == 0` (no hard line), `H <= S`, or `lines <= S` — logs nothing rather than fabricating a position ([budget.rs:187-189](../../../cli/src/guard/budget.rs#L187)).

The map is a three-way partition on that per-mille ([budget.rs:225-234](../../../cli/src/guard/budget.rs#L225)):
...
The switch is **off by default** and changes one repo only: `[guard] zone_tiers`, default `false` ([tier.rs:24-28](../../../cli/src/config/tier.rs#L24), [ce-toml.md:31](../ce-toml.md#L31)). Disarmed, the zone is feed-only — a `zone` line is written in every tier and `zone_assess` returns `None`, injecting nothing ([budget.rs:207-213](../../../cli/src/guard/budget.rs#L207)). Armed, the resolved tier is additionally stamped as `zone_tier` on the feed line, so the record says what the rule *did*, not only where the write landed ([budget.rs:207-209](../../../cli/src/guard/budget.rs#L207), [hookio.rs:31-34](../../../cli/src/hookio.rs#L31)).
...
- **Per-(rule, file, session) suppression.** A warn fires once per session per file per rule; the observe feed *is* the accumulator, consulted **before** the current event lands in it so a warn cannot read its own fresh line as "already warned" ([guard.rs:64-71](../../../cli/src/guard.rs#L64), [budget.rs:194-197](../../../cli/src/guard/budget.rs#L194), [hookio.rs:220-236](../../../cli/src/hookio.rs#L220)). Suppression is skipped entirely when `mode ∈ {deny, ask}` — enforcement is not context bloat and repeats every time it holds ([guard.rs:66-68](../../../cli/src/guard.rs#L66)); an armed zone `ask` likewise repeats, only `warn` is budgeted ([budget.rs:197](../../../cli/src/guard/budget.rs#L197)). Counting is conservative in the reporting direction: a clean probe line (`matches == 0`) and an unarmed zone line (`zone_tier` absent or `observe`) never count as "already warned", and any read or parse failure returns "not warned" — fail open toward *reporting*, the opposite bias from enforcement's fail-open ([hookio.rs:232-243](../../../cli/src/hookio.rs#L232)).
- **Token clip at the single emission throat.** `WARN_BUDGET_TOKENS = 200`, `STOP_BUDGET_TOKENS = 400`, converted to chars by one declared measurement constant `CHARS_PER_TOKEN = 4` ([hookio.rs:159-161](../../../cli/src/hookio.rs#L159)); the clip is char-boundary safe and appends `… (clipped; full report in .ce/observe.ndjson)`, pointing at the full on-disk record ([hookio.rs:175-188](../../../cli/src/hookio.rs#L175), applied at [guard.rs:158-162](../../../cli/src/guard.rs#L158)).
