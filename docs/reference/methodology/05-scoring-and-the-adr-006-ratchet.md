...
Seven axes, indexed by a fixed code, each producing one non-negative penalty count ([Score.hs:87-96](../../../core/app/CE/Verdict/Score.hs#L87)):

| code | axis | penalty definition | knob(s) and value |
|---|---|---|---|
| 0 | size | convex soft-zone penalty summed over `metricCode = 0` rows, floored once ([Score.hs:107-111](../../../core/app/CE/Verdict/Score.hs#L107)) | see soft zone below |
| 1 | complexity | count of `metricCode = 1` rows with `v > cocCeil` ([Score.hs:113-114](../../../core/app/CE/Verdict/Score.hs#L113)) | `cocCeil = 15` ([Cost.hs:139-140](../../../core/app/CE/Verdict/Cost.hs#L139)) |
| 2 | clone | count of sim rows with `kind <= 1` and `n * cloneDen >= d * cloneNum` ([Score.hs:116-118](../../../core/app/CE/Verdict/Score.hs#L116)) | `tsedNum/tsedDen = 85/100` ([Clone/Cost.hs:22](../../../core/app/CE/Clone/Cost.hs#L22), [Clone/Cost.hs:25](../../../core/app/CE/Clone/Cost.hs#L25)) |
| 3 | docdup | count of sim rows with `kind == 2` and `n * dupDen >= d * dupNum` ([Score.hs:120-121](../../../core/app/CE/Verdict/Score.hs#L120)) | `jaccardNum/jaccardDen = 80/100` ([Docdup/Cost.hs:27](../../../core/app/CE/Docdup/Cost.hs#L27), [Docdup/Cost.hs:30](../../../core/app/CE/Docdup/Cost.hs#L30)) |
| 4 | deadcode | count of pos rows with `reachIn == 0` and `indeg <= deadIndegCeil` ([Score.hs:123-124](../../../core/app/CE/Verdict/Score.hs#L123)) | `deadIndegCeil = 0` ([Cost.hs:146-147](../../../core/app/CE/Verdict/Cost.hs#L146)) |
| 5 | churn | count of churn rows with `rw + ap > 0` and `rw * rewriteDen >= (rw + ap) * rewriteNum` ([Score.hs:126-128](../../../core/app/CE/Verdict/Score.hs#L126)) | `rewriteNum/rewriteDen = 50/100` ([Cost.hs:81](../../../core/app/CE/Verdict/Cost.hs#L81), [Cost.hs:84](../../../core/app/CE/Verdict/Cost.hs#L84)) |
| 6 | graph_cycle | count of pos rows with `sccSize >= cycleFloor` ([Score.hs:130-131](../../../core/app/CE/Verdict/Score.hs#L130)) | `sccFloor = 2` ([Graph/Cost.hs:55](../../../core/app/CE/Graph/Cost.hs#L55)) |

All ratio thresholds are cross-multiplied rather than divided, so no rounding enters the predicates ([Score.hs:118](../../../core/app/CE/Verdict/Score.hs#L118), [Score.hs:121](../../../core/app/CE/Verdict/Score.hs#L121), [Score.hs:128](../../../core/app/CE/Verdict/Score.hs#L128)). Note the axis-2 and axis-3 thresholds are *not* owned by the verdict family: they are re-exported from the clone and docdup cost modules so that one authority defines "is a clone" ([Score.hs:20-23](../../../core/app/CE/Verdict/Score.hs#L20), [Cost.hs:7-10](../../../core/app/CE/Verdict/Cost.hs#L7)).

**Weights.** Each axis carries an effective weight: the wire's `weights` table supplies `[axisCode, w]` rows, and the first matching row wins; an unlisted axis takes `defaultWeight` ([Score.hs:138-141](../../../core/app/CE/Verdict/Score.hs#L138)), which is `1` ([Cost.hs:160-161](../../../core/app/CE/Verdict/Cost.hs#L160)) — equal weights are the opening stance. The same lookup that folds the score also builds the echoed table `0..6` returned in the reply, so the echo cannot diverge from the computation ([Score.hs:144-145](../../../core/app/CE/Verdict/Score.hs#L144), [Verdict.hs:116](../../../core/app/CE/Verdict.hs#L116)).

**The fold.** With `p_i` the axis penalties and `w_i` the effective weights ([Score.hs:150-157](../../../core/app/CE/Verdict/Score.hs#L150)):
...
`violCost = 10` ([Cost.hs:154-155](../../../core/app/CE/Verdict/Cost.hs#L154)) and `scoreScale = 1000` ([Cost.hs:168-169](../../../core/app/CE/Verdict/Cost.hs#L168)) — i.e. the score is an integer per-mille value opening at 1000, polarity higher-is-better ([DEVELOPMENT_PLAN.md:71](../../DEVELOPMENT_PLAN.md#L71)). `div` is Haskell floor division, and the `max 0` floor is structural, not a knob ([Cost.hs:166-167](../../../core/app/CE/Verdict/Cost.hs#L166)). `wTotal` is summed from the effective weights rather than declared, so a weight can never be silently dropped from the divisor ([Score.hs:148-149](../../../core/app/CE/Verdict/Score.hs#L148)); the same source comment states that validation refuses an all-zero weight table, making the divisor non-zero — that refusal lives in the boundary contract `CE.Verdict.Wire.violation` ([Verdict.hs:40-41](../../../core/app/CE/Verdict.hs#L40)), which I did not read for this section, so the specific check is asserted by comment, not verified here.

An over-cap request never gets a partial judgment: it returns a fully-shaped degraded reply with `score = 0`, empty axes, and `fail = true` with reason `verdict_too_large` ([Verdict.hs:192-225](../../../core/app/CE/Verdict.hs#L192)). Caps are `verdictNodeCap = 131072` nodes and `verdictRowCap = 524288` rows ([Cost.hs:177-181](../../../core/app/CE/Verdict/Cost.hs#L177)), the row count summing every fact and knob table plus the baseline's rows ([Verdict.hs:47-72](../../../core/app/CE/Verdict.hs#L47)).

### The size soft zone (axis 0)

Axis 0 is the one axis that is not a count. For a file of `x` LOC, with soft line `S` and hard line `H` ([Soft.hs:51-57](../../../core/app/CE/Verdict/Soft.hs#L51)):
...
The curve is exact `Rational`, and the *same* quadratic continues past `H` — monotone, no kink, no cap: `pMax` is the value *at* the hard line, not a ceiling ([Soft.hs:44-50](../../../core/app/CE/Verdict/Soft.hs#L44), [Cost.hs:113-119](../../../core/app/CE/Verdict/Cost.hs#L113)). Denying at `H` is Rust's job (scan fail tier, guard budget); here `H` only scales the curve ([Cost.hs:106-111](../../../core/app/CE/Verdict/Cost.hs#L106)). The degenerate `H <= S` branch reproduces the pre-v0.6 binary behaviour instead of dividing by zero or flipping the curve's sign ([Soft.hs:48-50](../../../core/app/CE/Verdict/Soft.hs#L48)).

Constants: `sizeHard = 750` ([Cost.hs:110-111](../../../core/app/CE/Verdict/Cost.hs#L110)), `sizePMax = 10` ([Cost.hs:118-119](../../../core/app/CE/Verdict/Cost.hs#L118)) — one file at the hard line weighs like ten of any other axis's violations. The per-file penalties are summed as `Rational` across all `metricCode = 0` rows and floored exactly once, at the axis boundary, so the wire's axes rows stay `Integer` ([Score.hs:107-111](../../../core/app/CE/Verdict/Score.hs#L107)).
