...
The two thresholds ([mod.rs:286](../../../cli/src/dedup/mod.rs#L286)–[289](../../../cli/src/dedup/mod.rs#L289), [winnow.rs:4](../../../cli/src/dedup/winnow.rs#L4)–[6](../../../cli/src/dedup/winnow.rs#L6)):

- **Guarantee threshold** `t = window + kgram - 1` ([mod.rs:299](../../../cli/src/dedup/mod.rs#L299)–[301](../../../cli/src/dedup/mod.rs#L301)). Any common substring of at least `t` normalized tokens contains at least `t - k + 1 = w` consecutive k-grams — one complete window — and since selection depends only on the contents of that window, both copies select the same minimum. Hence **≥ 1 shared fingerprint, always**. The positional dedup does not weaken this: it suppresses only a re-record of a position already emitted.
- **Noise threshold** `k = kgram`: no match shorter than `kgram` tokens can ever be reported, because a fingerprint is a whole k-gram.

Defaults: `kgram = 25`, `window = 26`, so `t = 26 + 25 - 1 = 50` tokens — chosen to align with the jscpd min-tokens default ([mod.rs:304](../../../cli/src/dedup/mod.rs#L304)–[313](../../../cli/src/dedup/mod.rs#L313)). The report filter defaults to exactly `p.guarantee()` ([mod.rs:142](../../../cli/src/dedup/mod.rs#L142)–[145](../../../cli/src/dedup/mod.rs#L145)); lowering it with `--min-tokens` is a calibration mode, and detection below `t` is opportunistic rather than guaranteed ([mod.rs:52](../../../cli/src/dedup/mod.rs#L52)–[55](../../../cli/src/dedup/mod.rs#L55)).

### 4. The inverted index

Fingerprints land in `fingerprints(hash, file_id, start_tok, start_line, end_line)` with `idx_fp_hash` and `idx_fp_file`, cascade-deleted from `files` ([schema.rs:45](../../../cli/src/dedup/schema.rs#L45)–[53](../../../cli/src/dedup/schema.rs#L53)). Line mapping at insert time is `start_line = toks[f.start].start_line` and `end_line = toks[f.start + p.kgram - 1].end_line` ([index.rs:329](../../../cli/src/dedup/index.rs#L329)–[330](../../../cli/src/dedup/index.rs#L330)) — the span of the k-gram, not of one token.

Invalidation is content-hash gated per file: `content_hash = fnv1a(src)`, and a match short-circuits the refresh entirely ([index.rs:112](../../../cli/src/dedup/index.rs#L112), [index.rs:123](../../../cli/src/dedup/index.rs#L123)–[125](../../../cli/src/dedup/index.rs#L125)); a change deletes and reinserts only that file's rows in one transaction ([index.rs:135](../../../cli/src/dedup/index.rs#L135)–[158](../../../cli/src/dedup/index.rs#L158)). The whole database is keyed by `SCHEMA_VERSION = 10` ([schema.rs:28](../../../cli/src/dedup/schema.rs#L28)) plus the meta tuple `(kgram, window, tokenizer_rev, graph_rev, struct_rev, docdup_rev)` ([schema.rs:130](../../../cli/src/dedup/schema.rs#L130)–[135](../../../cli/src/dedup/schema.rs#L135)); any mismatch wipes and rebuilds, so a parameter change cannot silently reuse stale fingerprints.
...
Each pair is normalized to `(a, b)` ordered by `(file, start_tok)` ([pairs.rs:219](../../../cli/src/dedup/pairs.rs#L219)–[223](../../../cli/src/dedup/pairs.rs#L223)), then verified against the two live token streams. A stored offset past the end of the live stream means the file changed after the index refresh: the anchor is skipped and counted in `stale_skipped`, never allowed to index out of bounds ([pairs.rs:230](../../../cli/src/dedup/pairs.rs#L230)–[235](../../../cli/src/dedup/pairs.rs#L235), [pairs.rs:88](../../../cli/src/dedup/pairs.rs#L88)–[91](../../../cli/src/dedup/pairs.rs#L91)).

`extend` computes the maximal **exact** common run around the anchor on token hashes ([pairs.rs:250](../../../cli/src/dedup/pairs.rs#L250)–[272](../../../cli/src/dedup/pairs.rs#L272)):

1. Backward: while `a0 > 0 && b0 > 0 && sa[a0-1].hash == sb[b0-1].hash`, decrement both ([pairs.rs:258](../../../cli/src/dedup/pairs.rs#L258)–[261](../../../cli/src/dedup/pairs.rs#L261)).
2. Forward: `cap = b0 - a0` when both sides are the same stream, else `usize::MAX` ([pairs.rs:263](../../../cli/src/dedup/pairs.rs#L263)); grow `len` while `a0+len < |sa|`, `b0+len < |sb|`, `len < cap`, and `sa[a0+len].hash == sb[b0+len].hash` ([pairs.rs:264](../../../cli/src/dedup/pairs.rs#L264)–[270](../../../cli/src/dedup/pairs.rs#L270)).
3. Return `(a0, b0, len)` when `len > 0` ([pairs.rs:271](../../../cli/src/dedup/pairs.rs#L271)).

The same-stream cap keeps the two ranges disjoint — periodic code reports adjacent segments instead of one self-overlapping range ([pairs.rs:245](../../../cli/src/dedup/pairs.rs#L245)–[249](../../../cli/src/dedup/pairs.rs#L249)).

Runs are then sorted into two sinks ([pairs.rs:236](../../../cli/src/dedup/pairs.rs#L236)–[242](../../../cli/src/dedup/pairs.rs#L242)): `len >= t` is a reportable run; `near_floor <= len < t` goes to the near-miss sink, which with the floor at `kgram` is exactly `25 <= len < 50` and is the T3 candidate source S1 ([pairs.rs:144](../../../cli/src/dedup/pairs.rs#L144)–[152](../../../cli/src/dedup/pairs.rs#L152)). `near_floor = usize::MAX` disables it ([pairs.rs:127](../../../cli/src/dedup/pairs.rs#L127)–[129](../../../cli/src/dedup/pairs.rs#L129)).

Reportable runs are mapped to lines via the run's endpoint tokens: `a_start = sa[a0].start_line`, `a_end = sa[a0+len-1].end_line` ([pairs.rs:276](../../../cli/src/dedup/pairs.rs#L276)–[285](../../../cli/src/dedup/pairs.rs#L285)).

Because periodic content yields one maximal run per offset, `dominant` drops any block whose **both** ranges sit inside a longer block of the same file pair: sort by descending `tokens`, keep a block only if no kept block contains it on both sides, then re-sort by `(a_file, a_start, b_file, b_start)` for a stable report order ([pairs.rs:195](../../../cli/src/dedup/pairs.rs#L195)–[216](../../../cli/src/dedup/pairs.rs#L216)).

### 7. The `min_distinct` low-diversity floor

`distinct` is the cardinality of the set of token hashes inside the verified run: `|{ sa[i].hash : i ∈ [a0, a0+len) }|` ([pairs.rs:277](../../../cli/src/dedup/pairs.rs#L277)–[281](../../../cli/src/dedup/pairs.rs#L281)). It is the literal-degeneracy signal: a data-row match such as a `LIT: (LIT, ...),` table has a tiny alphabet, while a real code clone is diverse ([pairs.rs:73](../../../cli/src/dedup/pairs.rs#L73)–[78](../../../cli/src/dedup/pairs.rs#L78)).

`DEFAULT_MIN_DISTINCT = 7` ([pairs.rs:109](../../../cli/src/dedup/pairs.rs#L109)). The calibration: across the fixture corpus plus cobra and requests, the arbitrated data-row false positives (status_codes rows, locale key sections, pygments style dicts) measured `distinct <= 6`, while arbitrated true clones measured `distinct >= 7` ([pairs.rs:97](../../../cli/src/dedup/pairs.rs#L97)–[102](../../../cli/src/dedup/pairs.rs#L102)). The same comment records that one 16-outlier false positive survives the floor — it buys precision, not purity.

The floor is applied **after** `dominant`, and the number of suppressed blocks is reported as `low_diversity_suppressed` rather than discarded silently ([pairs.rs:171](../../../cli/src/dedup/pairs.rs#L171)–[182](../../../cli/src/dedup/pairs.rs#L182), [pairs.rs:92](../../../cli/src/dedup/pairs.rs#L92)–[94](../../../cli/src/dedup/pairs.rs#L94)); `--min-distinct` overrides it ([mod.rs:43](../../../cli/src/dedup/mod.rs#L43), [mod.rs:144](../../../cli/src/dedup/mod.rs#L144)). Note that the floor filters blocks only — the near-miss sink keeps its own honest `distinct` but is not filtered by it ([pairs.rs:150](../../../cli/src/dedup/pairs.rs#L150)–[152](../../../cli/src/dedup/pairs.rs#L152)).

### 8. What the report carries

`ce.dedup-report/0.5.0` ([mod.rs:35](../../../cli/src/dedup/mod.rs#L35)) emits the blocks, the k-way groups aggregated from them, and a summary that restates the operating point — `kgram`, `window`, `min_tokens`, `min_distinct` — beside the transparency counters `hot_chained`, `stale_skipped`, and `low_diversity_suppressed` ([mod.rs:147](../../../cli/src/dedup/mod.rs#L147)–[162](../../../cli/src/dedup/mod.rs#L162), [mod.rs:206](../../../cli/src/dedup/mod.rs#L206)–[220](../../../cli/src/dedup/mod.rs#L220)). Every approximation the pipeline makes is therefore a number in the output, not an assumption in the reader's head.

**Not found in the source this run:** the `min_distinct` calibration numbers are cited from the comment at [pairs.rs:97](../../../cli/src/dedup/pairs.rs#L97)–[102](../../../cli/src/dedup/pairs.rs#L102), which attributes them to `DEDUP-CALIBRATION.md`; that document was not read for this section, so the per-corpus breakdown behind `distinct <= 6` vs `>= 7` is not reproduced here.
