...
Tokenization walks the tree-sitter tree and emits one `Token { hash, start_line, end_line }` per **leaf** ([tokens.rs:24](../../../cli/src/dedup/tokens.rs#L24)–[29](../../../cli/src/dedup/tokens.rs#L29), [tokens.rs:58](../../../cli/src/dedup/tokens.rs#L58)–[87](../../../cli/src/dedup/tokens.rs#L87)). The walk uses `ast::children`, which enumerates `child_count()` and therefore includes **anonymous** nodes ([ast.rs:43](../../../cli/src/scan/ast.rs#L43)–[45](../../../cli/src/scan/ast.rs#L45)) — punctuation and keywords are part of the alphabet, not discarded. Comment nodes are skipped as whole subtrees ([tokens.rs:63](../../../cli/src/dedup/tokens.rs#L63)–[65](../../../cli/src/dedup/tokens.rs#L65)).

Each leaf is classified into exactly three classes ([tokens.rs:108](../../../cli/src/dedup/tokens.rs#L108)–[116](../../../cli/src/dedup/tokens.rs#L116)):

| Class | Predicate | Bytes hashed |
|---|---|---|
| `Id` | `kind.ends_with("identifier")` ([tokens.rs:109](../../../cli/src/dedup/tokens.rs#L109)) | `ID_MARK = b"\x01ID"` ([tokens.rs:31](../../../cli/src/dedup/tokens.rs#L31)) |
| `Lit` | `is_literal(kind, spec)` ([tokens.rs:112](../../../cli/src/dedup/tokens.rs#L112)) | `LIT_MARK = b"\x02LIT"` ([tokens.rs:32](../../../cli/src/dedup/tokens.rs#L32)) |
| `Text` | otherwise ([tokens.rs:115](../../../cli/src/dedup/tokens.rs#L115)) | the node's **kind text**, `kind.as_bytes()` ([tokens.rs:93](../../../cli/src/dedup/tokens.rs#L93)) |
...
`is_literal` is leaf-kind only (composite kinds such as Go `composite_literal` never reach it, because `tokenize` recurses on any node with children first — [tokens.rs:66](../../../cli/src/dedup/tokens.rs#L66)–[69](../../../cli/src/dedup/tokens.rs#L69), [tokens.rs:121](../../../cli/src/dedup/tokens.rs#L121)–[122](../../../cli/src/dedup/tokens.rs#L122)). It accepts ([tokens.rs:123](../../../cli/src/dedup/tokens.rs#L123)–[132](../../../cli/src/dedup/tokens.rs#L132)):
...
`literal_delims` is per-language because a delimiter in one grammar is an operator in another: TypeScript `["\"", "'", "`"]` ([spec_launch.rs:109](../../../cli/src/scan/spec_launch.rs#L109)), Go `["\"", "`"]` — `'` is the rune delimiter but `rune_literal` lexes as one token ([spec_launch.rs:212](../../../cli/src/scan/spec_launch.rs#L212)–[213](../../../cli/src/scan/spec_launch.rs#L213)), Rust `["\""]` only, because `'` is the lifetime/label tick and classifying it as `LIT` made every `&'a str` signature a false clone driver ([spec_launch.rs:159](../../../cli/src/scan/spec_launch.rs#L159)–[161](../../../cli/src/scan/spec_launch.rs#L161)), Python `[]` because quotes surface as named `string_start` / `string_end` kinds ([spec_launch.rs:48](../../../cli/src/scan/spec_launch.rs#L48)–[49](../../../cli/src/scan/spec_launch.rs#L49)), Haskell `[]` because a string lexes as one `string` leaf with the quotes inside ([spec_hs.rs:79](../../../cli/src/scan/spec_hs.rs#L79)–[81](../../../cli/src/scan/spec_hs.rs#L81)).
...
Multi-piece literals collapse to one token, but **only within the same parent literal node** — `lit_parent` is the parent node id, and a piece whose parent matches merely extends the previous token's `end_line` ([tokens.rs:71](../../../cli/src/dedup/tokens.rs#L71)–[79](../../../cli/src/dedup/tokens.rs#L79), [tokens.rs:102](../../../cli/src/dedup/tokens.rs#L102)–[106](../../../cli/src/dedup/tokens.rs#L106)). Merging by lexical adjacency instead swallowed whole statements (a Python attribute docstring after a string assignment vanished into the previous `LIT`'s span).

Token hashing is FNV-1a over those bytes: `h = 0xcbf29ce484222325`, then per byte `h ^= b; h = h * 0x100000001b3` in wrapping u64 arithmetic ([tokens.rs:135](../../../cli/src/dedup/tokens.rs#L135)–[142](../../../cli/src/dedup/tokens.rs#L142)). It is dependency-free and stable across runs, which is required because the index persists.

Normalization semantics are versioned: <!--ce:ver:tokenizer_rev#digits-->`TOKENIZER_REV = 3`<!--/ce--> ([tokens.rs:21](../../../cli/src/dedup/tokens.rs#L21)) is stored in the index meta table and a mismatch clears every parser-derived table while the trend history stays — stamped with the revision that measured it and remeasured before reuse ([parser.rs:26](../../../cli/src/dedup/schema/parser.rs#L26), [mod.rs:4-8](../../../cli/src/trend/mod.rs#L4)) — so fingerprints from an older tokenizer can never mix with new ones ([schema.rs:109-119](../../../cli/src/dedup/schema.rs#L109), [schema.rs:174](../../../cli/src/dedup/schema.rs#L174)–[196](../../../cli/src/dedup/schema.rs#L196)).

Languages that do not fingerprint (`Lang::fingerprints() == false` — a grammar-less language such as Markdown or the scan-only arm, and HTML once its grammar lands; [lang.rs:190](../../../cli/src/scan/lang.rs#L190)) get no token stream and therefore **zero** fingerprint rows ([index.rs:145](../../../cli/src/dedup/index.rs#L145)–[162](../../../cli/src/dedup/index.rs#L162)). Fingerprints exist for Python, TypeScript, TSX, Rust, Go, and Haskell; the plan v2.30 reserved codes join as their own steps wire a grammar.

### 2. k-gram rolling hash

Winnowing consumes only the `hash` field of the token stream ([index.rs:150](../../../cli/src/dedup/index.rs#L150)–[151](../../../cli/src/dedup/index.rs#L151)). Let `t[0..n]` be that sequence and `k = p.kgram`.
...
The two thresholds ([mod.rs:285](../../../cli/src/dedup/mod.rs#L285)–[230](../../../cli/src/dedup/mod.rs#L230), [winnow.rs:4](../../../cli/src/dedup/winnow.rs#L4)–[6](../../../cli/src/dedup/winnow.rs#L6)):

- **Guarantee threshold** `t = window + kgram - 1` ([mod.rs:295](../../../cli/src/dedup/mod.rs#L295)–[303](../../../cli/src/dedup/mod.rs#L303)). Any common substring of at least `t` normalized tokens contains at least `t - k + 1 = w` consecutive k-grams — one complete window — and since selection depends only on the contents of that window, both copies select the same minimum. Hence **≥ 1 shared fingerprint, always**. The positional dedup does not weaken this: it suppresses only a re-record of a position already emitted.
- **Noise threshold** `k = kgram`: no match shorter than `kgram` tokens can ever be reported, because a fingerprint is a whole k-gram.

Defaults: `kgram = 25`, `window = 26`, so `t = 26 + 25 - 1 = 50` tokens — chosen to align with the jscpd min-tokens default ([mod.rs:303](../../../cli/src/dedup/mod.rs#L303)–[308](../../../cli/src/dedup/mod.rs#L308)). The report filter defaults to exactly `p.guarantee()` ([mod.rs:153](../../../cli/src/dedup/mod.rs#L153)–[151](../../../cli/src/dedup/mod.rs#L151)); lowering it with `--min-tokens` is a calibration mode, and detection below `t` is opportunistic rather than guaranteed ([mod.rs:54](../../../cli/src/dedup/mod.rs#L54)–[57](../../../cli/src/dedup/mod.rs#L57)).

### 4. The inverted index

Fingerprints land in `fingerprints(hash, file_id, start_tok, start_line, end_line)` with `idx_fp_hash` and `idx_fp_file`, cascade-deleted from `files` ([schema.rs:86](../../../cli/src/dedup/schema.rs#L86)–[94](../../../cli/src/dedup/schema.rs#L94)). Line mapping at insert time is `start_line = toks[f.start].start_line` and `end_line = toks[f.start + p.kgram - 1].end_line` ([index.rs:394](../../../cli/src/dedup/index.rs#L394)–[395](../../../cli/src/dedup/index.rs#L395)) — the span of the k-gram, not of one token.

Invalidation is content-hash gated per file: `content_hash = fnv1a(src)`, and a match short-circuits the refresh entirely ([index.rs:121](../../../cli/src/dedup/index.rs#L121), [index.rs:137](../../../cli/src/dedup/index.rs#L137)–[139](../../../cli/src/dedup/index.rs#L139)); a change deletes and reinserts only that file's rows in one transaction ([index.rs:152](../../../cli/src/dedup/index.rs#L152)–[185](../../../cli/src/dedup/index.rs#L185)). The whole database is keyed by <!--ce:ver:schema.index#digits-->`SCHEMA_VERSION = 16`<!--/ce--> ([schema.rs:55](../../../cli/src/dedup/schema.rs#L55)) plus the meta tuple `(kgram, window, tokenizer_rev, graph_rev, struct_rev, docdup_rev, similar_rev)` ([schema.rs:174](../../../cli/src/dedup/schema.rs#L174)–[179](../../../cli/src/dedup/schema.rs#L179)); any mismatch wipes and rebuilds, so a parameter change cannot silently reuse stale fingerprints.
...
The floor is applied **after** `dominant`, and the number of suppressed blocks is reported as `low_diversity_suppressed` rather than discarded silently ([pairs.rs:180](../../../cli/src/dedup/pairs.rs#L180)–[182](../../../cli/src/dedup/pairs.rs#L182), [pairs.rs:92](../../../cli/src/dedup/pairs.rs#L92)–[94](../../../cli/src/dedup/pairs.rs#L94)); `--min-distinct` overrides it ([mod.rs:45](../../../cli/src/dedup/mod.rs#L45), [mod.rs:155](../../../cli/src/dedup/mod.rs#L155)) — on the `--check` road only in the tightening direction: `--min-tokens` above 50, `--min-distinct` above 7, or `0`, are refused by name before any measurement or core contact ([budget.rs:21](../../../cli/src/dedup/budget.rs#L21)), so the ratchet always judges at the calibrated operating point or a stricter one. Note that the floor filters blocks only — the near-miss sink keeps its own honest `distinct` but is not filtered by it ([pairs.rs:159](../../../cli/src/dedup/pairs.rs#L159)–[162](../../../cli/src/dedup/pairs.rs#L162)).

### 8. What the report carries

<!--ce:report:dedup#schemaver-->`ce.dedup-report/0.5.0`<!--/ce--> ([mod.rs:217](../../../cli/src/dedup/mod.rs#L217)) emits the blocks, the k-way groups aggregated from them, and a summary that restates the operating point — `kgram`, `window`, `min_tokens`, `min_distinct` — beside the transparency counters `hot_chained`, `stale_skipped`, and `low_diversity_suppressed` ([mod.rs:154](../../../cli/src/dedup/mod.rs#L154)–[150](../../../cli/src/dedup/mod.rs#L150), [mod.rs:264](../../../cli/src/dedup/mod.rs#L264)–[report.rs:16](../../../cli/src/dedup/report.rs#L16)). Every approximation the pipeline makes is therefore a number in the output, not an assumption in the reader's head.
