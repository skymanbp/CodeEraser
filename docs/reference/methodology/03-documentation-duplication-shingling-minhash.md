...
- **Markdown** paragraphs are maximal runs of *adjacent* paragraph lines ([segments.rs:55](../../../cli/src/docdup/segments.rs#L55)); a non-paragraph line breaks adjacency, so no state beyond the last line number is carried ([segments.rs:65-67](../../../cli/src/docdup/segments.rs#L65)). A line is paragraph content iff it is non-empty and does not start with `#` (ATX heading) or `|` (table row), is not a bare list marker, and is not an HTML-block line ([segments.rs:98-104](../../../cli/src/docdup/segments.rs#L98)). `html_line` is the CommonMark HTML-block start condition at line level: `<` followed by an ASCII letter or `/` ([segments.rs:109-115](../../../cli/src/docdup/segments.rs#L109)). Bare markers are `-`, `*`, `+`, or a digit run followed by `.` or `)` ([segments.rs:117-123](../../../cli/src/docdup/segments.rs#L117)).
- **Comments and docstrings** come off one tree-sitter parse of docdup's own ([segments.rs:139-162](../../../cli/src/docdup/segments.rs#L139)). Contiguous same-column comment nodes merge into one block — `mergeable` iff the previous node ended on row `r−1` at the same start column ([segments.rs:180-196](../../../cli/src/docdup/segments.rs#L180)). Only Python has a docstring convention here (`module`, `function_definition`, `class_definition` hosts whose body's first named child is a bare-string expression statement); JSDoc and Rust `///` arrive lexically as comments ([spec.rs:88-90](../../../cli/src/docdup/spec.rs#L88), [segments.rs:167-175](../../../cli/src/docdup/segments.rs#L167)).

Markdown segments may come from nothing but `md::masked_content_lines` ([segments.rs:4-6](../../../cli/src/docdup/segments.rs#L4)) — the fence + HTML-comment + inline-code triple mask ([md.rs:40-47](../../../cli/src/graph/md.rs#L40)). The byte mask rides *into* wordization rather than being re-derived, so the judge can never see text the detector masks. Line classification itself runs on the unmasked characters only ([segments.rs:84-91](../../../cli/src/docdup/segments.rs#L84)).
