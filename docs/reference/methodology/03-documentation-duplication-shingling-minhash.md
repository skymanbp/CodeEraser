...
- **Comments and docstrings** come off one tree-sitter parse of docdup's own ([segments.rs:143-166](../../../cli/src/docdup/segments.rs#L143)). Contiguous same-column comment nodes merge into one block — `mergeable` iff the previous node ended on row `r−1` at the same start column ([segments.rs:184-200](../../../cli/src/docdup/segments.rs#L184)). Only Python has a docstring convention here (`module`, `function_definition`, `class_definition` hosts whose body's first named child is a bare-string expression statement); JSDoc and Rust `///` arrive lexically as comments ([spec.rs:91-100](../../../cli/src/docdup/spec.rs#L91), [segments.rs:171-179](../../../cli/src/docdup/segments.rs#L171)).
