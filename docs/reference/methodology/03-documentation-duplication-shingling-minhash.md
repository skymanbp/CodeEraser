...
Three document-text kinds are extracted, frozen as position codes `["md_para", "comment_block", "docstring"]` = `0, 1, 2` ([spec.rs:49](../../../cli/src/docdup/spec.rs#L49), [spec.rs:50-52](../../../cli/src/docdup/spec.rs#L50)).

- **Markdown** paragraphs are maximal runs of *adjacent* paragraph lines ([segments.rs:59](../../../cli/src/docdup/segments.rs#L59)); a non-paragraph line breaks adjacency, so no state beyond the last line number is carried ([segments.rs:69-71](../../../cli/src/docdup/segments.rs#L69)). A line is paragraph content iff it is non-empty and does not start with `#` (ATX heading) or `|` (table row), is not a bare list marker, and is not an HTML-block line ([segments.rs:102-108](../../../cli/src/docdup/segments.rs#L102)). `html_line` is the CommonMark HTML-block start condition at line level: `<` followed by an ASCII letter or `/` ([segments.rs:113-119](../../../cli/src/docdup/segments.rs#L113)). Bare markers are `-`, `*`, `+`, or a digit run followed by `.` or `)` ([segments.rs:121-127](../../../cli/src/docdup/segments.rs#L121)).
- **Comments and docstrings** come off one tree-sitter parse of docdup's own ([segments.rs:143-166](../../../cli/src/docdup/segments.rs#L143)). Contiguous same-column comment nodes merge into one block — `mergeable` iff the previous node's last *content* row is `r−1` at the same start column ([segments.rs:188-206](../../../cli/src/docdup/segments.rs#L188)); the content row is the end row unless the node ends at column 0 of a later row, which tree-sitter-rust's doc-comment node (`///`, `//!`) does, so a Rust doc block is one segment whose `end_line` is its last line, not one segment per line ending a row late (`DOCDUP_REV` 5, v2.28; [segments.rs:208-215](../../../cli/src/docdup/segments.rs#L208)). Only Python has a docstring convention here (`module`, `function_definition`, `class_definition` hosts whose body's first named child is a bare-string expression statement); JSDoc and Rust `///` arrive lexically as comments ([spec.rs:91-100](../../../cli/src/docdup/spec.rs#L91), [segments.rs:171-179](../../../cli/src/docdup/segments.rs#L171)).

Markdown segments may come from nothing but `md::masked_content_lines` ([segments.rs:4-6](../../../cli/src/docdup/segments.rs#L4)) — the fence + HTML-comment + inline-code triple mask ([md.rs:50-56](../../../cli/src/graph/md.rs#L50)). The byte mask rides *into* wordization rather than being re-derived, so the judge can never see text the detector masks. Line classification itself runs on the unmasked characters only ([segments.rs:88-95](../../../cli/src/docdup/segments.rs#L88)).
...
- `license_header` requires all three of: the segment is the file's **first** comment block, `start_line <= LICENSE_HEAD_LINES` where `LICENSE_HEAD_LINES = 5` ([spec.rs:22](../../../cli/src/docdup/spec.rs#L22)), and some line contains one of five markers — `SPDX-License-Identifier`, `Licensed under the Apache License`, `Copyright (c)`, `Permission is hereby granted`, `MIT License` ([spec.rs:56-62](../../../cli/src/docdup/spec.rs#L56), applied at [exempt.rs:42-45](../../../cli/src/docdup/exempt.rs#L42)).
- `inline_allow` requires the marker `ce:allow(docdup)` ([spec.rs:88](../../../cli/src/docdup/spec.rs#L88)) *plus* a non-empty `-- <why>` tail; a bare marker exempts nothing and is itself ledgered as `allow_missing_why` while the segment stays live ([exempt.rs:49-58](../../../cli/src/docdup/exempt.rs#L49), predicate at [exempt.rs:69-73](../../../cli/src/docdup/exempt.rs#L69)).
...
**Line-level strips** apply to comment/docstring segments only; `md_para` lines are passed through untouched by all three ([exempt.rs:87-91](../../../cli/src/docdup/exempt.rs#L87)):

1. **Fenced code** — ` ``` ` or `~~~` after stripping comment decoration `['#','/','*','!',' ']`, toggling with XOR so an unclosed fence honestly strips to segment end ([exempt.rs:92-97](../../../cli/src/docdup/exempt.rs#L92), [exempt.rs:111-114](../../../cli/src/docdup/exempt.rs#L111)).
2. **Skeleton rows** — a decoration-stripped line that is all `-` and at least 3 chars, or one starting with any of the 16 `SKELETON_PREFIXES` (`Args:`, `Arguments:`, `Returns:`, `Raises:`, `Yields:`, `Parameters`, `Attributes:`, `Example:`, `Examples:`, `Note:`, `:param `, `:return`, `:rtype`, `@param`, `@returns`, `@throws`) ([spec.rs:67-84](../../../cli/src/docdup/spec.rs#L67), [exempt.rs:118-127](../../../cli/src/docdup/exempt.rs#L118)).
3. **Overlong lines** — trimmed visible char count `> DOC_LINE_CAP`, `DOC_LINE_CAP = 200` ([spec.rs:45](../../../cli/src/docdup/spec.rs#L45), [exempt.rs:100](../../../cli/src/docdup/exempt.rs#L100)). Rationale in-source: hard-wrapped comment prose runs under ~120 chars while the audited false-positive lines (regex literals, inline snapshots) ran 300+/600+ ([spec.rs:36-40](../../../cli/src/docdup/spec.rs#L36)).

### 3. Wordization and shingle construction

A **word** is a maximal run of `char::is_alphanumeric()` characters — a combining mark (`General_Category=Mark`) does not end the run it sits on — lowercased and NFC-composed before hashing, so canonically equivalent spellings ("café" typed NFC or NFD) hash alike instead of yielding disjoint shingle sets ([shingle.rs:25-39](../../../cli/src/docdup/shingle.rs#L25), the one hash throat at [shingle.rs:44-47](../../../cli/src/docdup/shingle.rs#L44); `DOCDUP_REV` <!--ce:ver:docdup_rev#digits-->5<!--/ce-->). A masked byte or any other non-alphanumeric character terminates the current word, as does end of line. Each word is hashed with the repo's one FNV-1a:
...
Only the set is cached: `docsegs.shingles` is the sorted deduped `u64`s in little-endian ([mod.rs:34-39](../../../cli/src/docdup/mod.rs#L34), encoder at [mod.rs:119](../../../cli/src/docdup/mod.rs#L119)). <!--ce:ver:docdup_rev#digits-->`DOCDUP_REV = 5`<!--/ce--> sits in the meta cache key so a semantics change wipes stale rows ([mod.rs:21-30](../../../cli/src/docdup/mod.rs#L21)).
