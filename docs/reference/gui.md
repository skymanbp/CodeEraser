...
| **Erase** | `ce.erase-plan` + a rendered diff | the deterministic two-phase eraser's face: preview **is** the plan (eraseable rows with provenance, advisories with named reasons, the unified diff rendered from the hashed plan); Apply stays hidden until a preview exists; **Audit log** lists `.ce/erase-log.ndjson` (`ce.erase-trail-report`) — the same records `ce erase --log` prints, refused lines named by number |
