...
| `search_roots` | *(absent)* | A table of language name → root-relative directories that language's ladder searches after a reference's own rung and before its build configuration (plan v2.30): `c = ["include"]` is where a C or C++ `#include "x.h"` looks when the including file's directory holds no `x.h` (two declared roots holding two different files refuse as ambiguous; `compile_commands.json` answers after them). The keys are `c` (C++ shares it), `lua`, `java`, `r`, `html`, each read by its language's step; any other key is refused at load, and a directory holding no walked file is refused by the walk |
