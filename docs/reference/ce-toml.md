...
| `search_roots` | *(absent)* | A table of language name → root-relative directories that language's ladder consults (plan v2.30): `c = ["include"]` is where a C or C++ `#include "x.h"` looks when the including file's directory holds no `x.h` (two declared roots holding two different files refuse as ambiguous; `compile_commands.json` answers after them); `java = ["src/main/java"]` settles a class or a package that two walked files or directories answer — one package in two source roots — by the one under a declared directory, where the ladder otherwise refuses as ambiguous; `lua = ["vendor"]` joins the directories a `require` searches (the root, `src`, `lua` and every template the tree's own files write into `package.path`), each tried as `name.lua` then `name/init.lua`; `r = ["lib"]` is where a `source("x.R")` looks after the sourcing file's directory and the root; `html = ["site"]` is the deployment root a page's root-relative `/x` resolves under, ahead of the root the page's own URL derives (its canonical, `og:url` or own-language `hreflang` alternate) and of the one ancestor directory holding the path (two declared roots holding it refuse as ambiguous). The keys are `c` (C++ shares it), `lua`, `java`, `r`, `html`; any other key is refused at load, and a directory holding no walked file is refused by the walk |
