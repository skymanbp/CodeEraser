# Changelog 归档册（v1.4.x）

> v1.4.0–v1.4.1 两条版本条目。2026-09-05 从 [CHANGELOG.md](../CHANGELOG.md) 迁出：
> 那一册第三次抵到 750 行硬线，而第一归档册 [CHANGELOG-ARCHIVE.md](CHANGELOG-ARCHIVE.md)
> （685 行）装不下这 314 行，按仓规「拆分优先于豁免」另起一册。条目逐字节搬家，
> 未改写；记录义务与两类内容的定义仍由原册声明。
