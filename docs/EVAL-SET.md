...
元对齐）的量化基线；O48 续测见 [EVAL-SET-M5-3.md](EVAL-SET-M5-3.md#声明级搬迁o48)。
...
退役（整文件）：`eval_extract/`（M4 冻结驱动+freeze/scan，manifest 哈希链封册）、`eval_prelabel.rs`、`fpr_replay.rs`（FPR-REPLAY.md 封册为最终记录；2026-09-05 计划 v2.29 步 9 O49 以子仓 `it/fpr_replay.rs` 复立为常设 `#[ignore]` 腿——每条拦截对父版基线与提交整体落地各探一次，账本仍是 FPR-REPLAY.md）、`perf_budget.rs`（PERF-BUDGET.md 同）、`crosscheck_rca.rs`（crosscheck fixtures **保留**=graph 宇宙设计内负对照，活门在读）、`regen_tables.rs`（表变更时从历史复活再冻结）。**v0.5.0 加码批（2026-08-20 用户拍板）**：一次性 attainment/决策仪器整件退役——`eval_ablation`、`eval_baseline`、`eval_commit_baseline`、`eval_commits`、`eval_commit_labels`、`eval_l1`、`eval_churn_ledger`、`eval_t3_recall`、三族 sample/audit 装配腿六件与 `eval_docdup_oracle` 测试 + 伴生 `eval_ablation_parts`/`eval_graph_sample_parts`；留任 = `eval_l2`（主杠，`eval_commit_review/` 因其活读保留）+`eval_labels`+`fpr_fourclass`（M4 主门）+三族 universe 漂移网+三族 precision 回归门（直读冻结 sample/oracle JSON，工件保留）+provenance 双门；克隆预算随删 169→167。
退役（文件内）：22 个混载仪器的 `generate_*`/scout/重放半场共 27 fn+专属支援件约 77 项（零引用不动点清扫；编译器+全量测试双证活门无损）。
冻结工件退役：`manifest-v1.json`（6.6k 行，M4 抽取清单——其仪器链整体封册后读者归零）；v0.5.0 加码批续退 11 件读者归零工件（baseline-l0、commit-ablation×3、commit-baseline-l1×3、t3-recall×3、churn-ledger）。首拟名单曾含 t3-universe 四语料件+t3-candidates-v1，**活门 `t3_candidates_consistent` 当场红回**——`each_frozen_doc` 按家族前缀动态拼路，字面路径普查看不见此类读者；冻结件退役判据自此为"删后全量测试绿"实证，不凭引用扫描（加码批同判据：56 二进制全绿）。
**再生成**：rev 变更需重冻结时 `git show <父提交>:cli/tests/<仪器> > cli/tests/<仪器> && git archive <父提交> cli/tests/eval_support | tar -x` 复活生成器**连同其同代支撑**（2026-08-24 死码清理批起，生成器专用助手已随退役仪器出仓，单取仪器文件对今日支撑编译不过；`cli/tests` 自 9bedcc4 起是 submodule，配方只读超仓历史、不写索引——对着 submodule 路径 `git checkout <sha> -- <路径>` 会静默把 gitlink 换成 blob），跑毕纯 `rm -rf` 退役。历史即真源（与 trend 缓存同立场）。
