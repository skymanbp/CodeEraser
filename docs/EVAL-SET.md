...
**再生成**：rev 变更需重冻结时 `git checkout <父提交> -- cli/tests/<仪器> cli/tests/eval_support` 复活生成器**连同其同代支撑**（2026-08-24 死码清理批起，生成器专用助手已随退役仪器出仓，单取仪器文件对今日支撑编译不过），跑毕重退役。历史即真源（与 trend 缓存同立场）。
