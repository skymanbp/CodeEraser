...
| S3 漂移错位 | 持有错位文件（引用多数邻域在别目录，社区错配）的**目录**计数；文件级谓词不变（outside ≥ min 且 > 2×inside，逐文件测量），过线目录计 1（修正案①） | graph 边表既有 | 同上聚合 |
...
> **修正案①（用户拍板 2026-08-19）**：S3 罚分单位由错位文件数改为
> 持有错位文件的**目录数**（去重）。依据 = 评审实测：score 把七轴罚分
> 等权加进一个和，唯 S3 按文件计——500 个错位文件的一个垃圾抽屉能把
> 整份结构分打到 0，淹没其余全部轴；改后与 S6 体例一致（过线目录计 1）。
> **文件级覆盖映射**（拍板附带要求）：逐文件谓词与 fFileRefs 测量不变；
> 散落=每个受害目录各计 1（散得越广罚越多）；单目录文件爆炸=S0 扇出
> +S4 文档辖区；堆叠/克隆=S6 与 dedup 辖区——一现象一轴，不双计。
> findings 行与 wire 形不变；golden 与手算电池随判决重切。
