...
| Rust | `mod foo;`→dir/foo.rs \| dir/foo/mod.rs；crate 根出 [[bin]]/lib.rs——唯一建模块结构的 rung | `use crate::a::b::C` 走 **R1 导出**的树 | self::/super:: 同树 | workspace 成员或 path= 依赖→其根文件；registry 依赖 ⇒ External | `#[path="…"]` 字面量回 R1；宏生成 mod/use ⇒ macro；`pub use` 绑 **≤1 跳**取定义点（未绑/歧义 ⇒ 保文件级边；修正案 2026-08-18，与 GT/验收「取定义点」对齐），标 via_reexport |
...
| 2e | schema v4、symbols/sites/edges、resolve_key、Markdown 入 files、两处 dedup/ 拆分 | (i) `ce dedup --check` 恰报 201 块（2026-08-12 兑现注：意图=md 入 files 零扰动，由 set-diff 证明——schema/md 改动本身 +0 块且根治 −1 块；批内新测试脚手架 +4 被自家棘轮抓获，open_idx/put/corpus 咽喉化清偿后 201→188，预算同 PR 棘轮至 188，"恰报"对新预算成立）；(ii) report.golden.json 字节同一；(iii) 增量≡全量；(iv) index.rs/dedup mod.rs/server.rs/main.rs 全 ≤300；(v) 100k LOC 全量重解析 <2s。红：任一 |
