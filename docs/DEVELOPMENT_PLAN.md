...
- **旋钮围栏（v2.14 起；4.2.0 `classDigest` 只钉 `[[rules.class]]`，6.0.0 拓宽为整份解析 ce.toml 并改名 `knobsDigest`，v2.18 步 #14 O39 起为规范树 = 与出厂默认不同的**有效**旋钮集，类名是标签不入树〔步 #16 O42 收口〕）**：基线另存标量 `knobsDigest`。
