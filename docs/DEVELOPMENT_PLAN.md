...
- 二进制分发路径**唯一化（A9a）**：仓库 `bin/` 只放轻量启动脚本；真身二进制由
  ...
  **HTTPS + SHA256 pinned 在插件清单内**，校验失败拒绝执行并明示。三平台二进制
  ...
  air-gapped 模式：允许用户手动放置二进制 + 本地校验。代码签名/公证**裁定不做**
...
**依赖**：M2←M1；M3←M2；M4←M3；M5-2←M4（churn 是三信号一腿，**串行**——A4）；M5-3A←M5-2；M5-3B←M5-3A；M6 可与 M5 并行；M7 收尾；M8←M7（成长轨，契约在册——v2.2；G1 IP 材料可与 M7 并行，商标先于 P6）；M9←M8（批 0 先行，批 1–7 依批序，批 6 可与 4/5 并行；批 8←批 7，批 9←批 8，终扫←批 9——v2.10）。总计粗估 4–6 个月。
