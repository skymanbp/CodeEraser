...
| 新入场（复扫 2026-08-29，亲核 README）：[dupehound](https://github.com/Rafaelpta/dupehound)（89★，同栈 tree-sitter + winnowing k10/w17 + Jaccard，15 语言）、[nose](https://github.com/corca-ai/nose)（69★，语义 IL）、[vibeguard](https://github.com/majiayu000/vibeguard)（41★）、[slopo](https://github.com/rafal-qa/slopo)（366★，embedding） | dupehound `check` = 变更函数 diff 门（退 1）+ MCP + 月度 `history`；nose `base=origin/main` PR 门；vibeguard = Claude Code/Codex/Gemini PreToolUse **写入时拒绝**（800 行 / 未读先改 / 破坏性 bash）；slopo `review` 变更 diff 报告 | 无一把克隆判定放进写入当轮：dupehound/nose/slopo 是提交/PR 时批扫，vibeguard 有拦截无查重（重名符号仅 warn）；三信号 join 与四分类仍无人做 |
...
| `complexity` | Cognitive Complexity 主判罚；Cyclomatic 辅助 | CoC 15（Sonar S3776）；CC 10–15（Sonar S1541=10 / lizard=15）。证据边界如实声明：ESEM 2020 元分析中 CoC 仅在理解耗时（r=0.54）与主观评分轴有支持，正确率轴无支持（r=−0.13 CI 跨零）；arXiv 2303.07722 中 CC 略优于 CoC。选 CoC 主判罚的理由是其对嵌套的惩罚正对准"堆叠"形态，而非"已证明的可维护性代理"。**v2.24（2026-09-01）补 `cognitive_fail`**：此前复杂度轴**没有任何绝对上限**——`CE.Scan.Cost.gradeTable` 的 code 2..6 fail 列字面量为 0、guard 对复杂度零命中（§4.2 明令 PreToolUse 不做 AST，故写入时不接）、ADR-006 棘轮按设计只止涨（新实体 bootstrap 取自身值，`Ratchet.hs` 自陈 not a violation）。本键把 fail 档补齐，默认 **0 = 无硬线**（gradeTable 的既定语义）、纯 opt-in——理由就是本行上面那条证据边界：不为一个自陈"正确率轴无支持"的指标设默认拦截。作用面 = `ce scan` fail 档 + `[[rules.class]] knobs.cognitive_fail`；**不进评分**（score 复杂度轴仍只读 `cocCeil`），故分数与 1.4.x 完全可比 | M1 |
...
- 退出码：`0` 通过 / `1` 违规 / `2` 内部错误；`--fail-under <score>`（与棘轮合成语义见 ADR-006）。`ce update` 例外——退出码即判定（0 最新 / 1 有更新 / 2 未知，见 §6 v2.20 条与 CHANGELOG v1.3.0）。
...
- **旋钮围栏（v2.14 起；4.2.0 `classDigest` 只钉 `[[rules.class]]`，6.0.0 拓宽为整份解析 ce.toml 并改名 `knobsDigest`，v2.18 步 #14 O39 起为规范树 = 与出厂默认不同的**有效**旋钮集，类名是标签不入树〔af45fcf 复核收口〕）**：基线另存标量 `knobsDigest`。
  digest 不符 → 棘轮**硬拒**并要求具名动作——`CE_ACCEPT_FENCE=1` 只接受围栏条件、上限与旧值取 min（v2.18 步 #14 O35），`CE_ACCEPT_BASELINE=1` 整体重立——规则集变更不得被无声吸收。
  围栏在场时容差可按类声明（`ratchet_tolerance`，以及仅对 CoC 行的 `cognitive_ratchet_tolerance`〔6.4.0〕，vendored/夹具可为 0=冻结）；棘轮行仍三列只记树事实，
  进基线的是指纹而非旋钮。缺省无 digest = 判决等于出厂默认，这样的仓字节恒等（K11）。
...
验收 = 产品判决面字节等价（上报集/退出码/报告行，golden 全绿）+每片反事实证表承重；wire 按 proto minor 加性演进。占比提升是副产品，禁止为占比写代码。全档：reviews/2026-08-17-adr-008-policy-dsl.md（git 历史）。**细则第二期 = 规则包 DSL v1 路径类分参（2026-08-24 拍板，v2.13）**：glob→classId 指派居 Rust 测量侧（与 exclude 同一解析器 `scan/globs.rs`，v2.18 步 #14 O42/O20 合一），类尺求值居 Haskell（`classKnobs` 数据表，码域 = {0,1,2} ceilings 类影子 + 3/4 棘轮容差〔4 = 仅 CoC，6.4.0〕）；类名与 glob 永不过线，线上只走整数（continuous 第 4 列 classId + classKnobs 三元组）；声明序首中、classCap 64、baseline 永三列；四片 P1→P4 与反事实证表 C1–C9 为验收正典（全文见 v2.13 banner 条）。**细则第三期 = 符号事实搭 import 绑定（2026-08-24 拍板，v2.14）**：符号的导出性是每文件**本地语法事实**（`pub`/`export`/`__all__`），指派居 Rust 测量侧；符号存活性求值居 Haskell（4.1.0 先上 `symbols` 一表派生文件层导出位，判决码域沿用 graph/1 四态不新造；步 3b 符号层顾问 = **v2.17 L 轮甲批**（口径对抗审查册 v9 **已封版 2026-08-27**——提及口径 / 约定式可达免疫 / 嵌套可见性三题定案，见 v2.17 banner 条；`symEdges` 终裁 = **删**，用户拍板 ①），缓议因由见 v2.14 K7 条）。判据仍是 §5.9.2 一票否决——符号**名**属文本形物故永不过线，线上只走下标；边只从 import 绑定派生，不由名字搜索铸造（R6 判例：同名匹配铸边精度 0.576，永久关闭不复议），且**须在目标文件的 symbols 表命中声明**方成符号边，否则退回文件级边（对抗验证 2026-08-24：语法无法分辨叶子是声明还是子模块/再导出，详见 v2.14 ① 条）。**细则第四期 = 递归增量的调用关系（2026-08-31 两拍板，v2.23）**：CoC 的递归 +1（S3776 §1，M1 起挂账至今）口径**站规范侧**——白皮书 v1.7 p.8 与 Appendix B1 皆写 `each method in a recursion cycle, whether direct or indirect`，Change log v1.1（2017-02-06）「include indirect recursion」证其为刻意扩宽；而 SonarSource 自家 java/python/js 三 analyzer 均零实现（各自源码 `recurs` 命中 0），故落地后与 SonarQube 真实分数的系统性差走 crosscheck DIVERGENCES「归因保留」栏。形态 = **能证明的那部分调用关系上的精确 SCC**：调用边是**每解析单元内的词法作用域事实**（各语言按其文法解析，判据**不是**「体内出现自己的名字」——实测同名不同类型是主要假阳性源），指派居 Rust 测量侧，解析不到即**不建边**（欠计单调，宁可少计不可错计）；环的判定居 Haskell，SCC 用与图面同一个 `Data.Graph.scc`，自环语义沿用 `CE/Graph/Cycles.hs` 已钉的 `cyclic [v] = member (v,v) arcs`（直接递归 = 环长 1 的特例），但**不复用 `CE.Graph.Cycles` 本体**——它自陈 cycles are REPORTED never judged（RG9），本期是判决，两面不得混。**跨文件环具名不做**，据两个量过的数：名字铸边精度 0.576（R6，永久关闭不复议）、绑定派生符号边召回 ~23%（v2.14 K7，symEdges 因此退役）——低召回对死码用途致命而对本用途只是欠计，低精度才对本用途致命（误加 +1 直接污染 `ce check` 分数与 E01 敕令），故不为 23% 召回复活已删的符号面。**`ce scan` 自足不破**：CoC 只由该文件这一份 AST 决定，永不依赖索引/图是否跑过，否则同一文件会因索引冷热得出不同分数。wire = `scan/1` 加性表 + 新增可测不变量「一个文件的行不得跨 chunk」——现有分块论证 `rows grade independently` 对跨行判决不成立。

### 5.9 安全与隐私（A9，上市场的准入条件）

1. **网络承诺**：`ce` 与 `ce-core` 在分析路径上**绝不联网**；网络行为只有两种且都可关：SessionStart
   二进制下载，与 `ce update` 检查（v2.20：读 GitHub 发布索引 + tag 上已提交的 `manifest.env`，不发送任何项目数据；SessionStart 通知缓存一天，`CE_UPDATE_CHECK=0` 关闭；`--yes` 只放置 pin 校验通过的字节——O83 自更新以此逆转延期，信任锚 = pin 提交而非签名）。embedding 特性仅限本地模型；任何云 API 需按仓库显式 opt-in。
...
| **M5-2** 图 + 死码 | `graph`（独立子系统，验收对齐 ops-codegraph-tool 锚点；调用边=import-绑定层，R6 全仓同名匹配为条件项：须独立 100 调用点审计 ≥90% 方开——2026-08-12 拍板；**实测铸边精度 38/66=0.576 < 0.90，按本判据永久关闭（RG10 收案 2026-08-17，见 ADR-008）**）、`deadcode` | 3–4 周 ± | import 边 precision ≥ 90%（抽样人工核对 100 条，覆盖五门首发语言——D2-4；TS/Go 语料=crosscheck 已钉 zod/cobra commit）；`unreferenced_public` 独立报告类不并入 dead；本仓库 deadcode 发现全处置；core 判定不变量属性电池入 CI（2g 起——2026-08-12 拍板） |
...
| **K–L** 收口轨（K 轮 v2.14 **已交付 2026-08-26** 随 v1.2.0；K+1 CI 减重批 7dc391d→cd4ea86 用户拍板；**L 轮 v2.17 立项 2026-08-26，甲-1 封版 2026-08-27**） | 甲 步 3b 符号层顾问全链 + 乙 围栏收尾 19 条 + 丙 文档/记账/实录清欠 9 条（全文见 v2.18 banner 条）+ **v2.18 两步**：减法批（7 条 ≈116 行 + 3 缺陷，**已落 2026-08-28**）+ 测试子仓（cli/tests + gui/tests → skymanbp/CodeEraser-tests submodule，**已落 2026-08-28**）；具名后置束 M（评分/评测 12 + 产品小项 9）/ N（分发 20）/ 证据门 4 **经 v2.22 结项令一次性裁定不做（2026-08-31，45/45 具名）**，读者可见的永久立场已写入两个 README 的「已知限制」 | 3–4 周 ± | 甲：口径对抗审查册**已封版**（v9，2026-08-27：九轮双把、第七至九轮核验者 Workflow 零 REFUTED、第九轮双把零 breaks；定案见 v2.17 banner 条）→ 片序 (1)→(8)：symEdges 退役（SCHEMA 14 wipe，**已落 dd20947**）→ 可见性三位字 + bit 0 三修 T3/H2+H4/H5（GRAPH_REV 11，wire 掩码 bit 0，K27 差分入 CHANGELOG，**已落 2026-08-27**）→ mentions 表（自有第二 walk + 三发射器分词 + 两加性表存 fnv1a64、`MENTION_REV` 1 自有版本行、自有入口 `ce graph --mentions`，K39–K42 四腿 + secrets 表八条加宽同表，**已落 2026-08-27**）→ conv 类别字列（`symbols.conv` 存 AST 半八位、名表半 wire 时算，`mention_name` 域抽取器 + `ce:allow` 解析合一 `crate::allow`，GRAPH_REV 12，K24/K25/K28/K37 生产者半，**已落 2026-08-27**）→ `mounts` 表生产者 + cabal 两字段 + TS `export_star` 站点（GRAPH_REV 13，冻结自仓切片一行改签，K30 生产者半 e2e 六格 + 再导出/Go/cabal/无 lib 包，R5 梯级召回界具名——用户裁同日入提取面补强步作梯级修正案〔见本条 v2.17 提取面补强句〕，**已落 2026-08-27**）→ graph/1 6.2.0 加性两表 `unmentioned`/`mounts`（`symbols` 不动、legacy 请求字节恒等）+ `export_unmentioned` golden 反事实（约定式可达名零误报腿 K33；`CE.Graph.Advisory` 自有类 + Rust 候选生产者 `mention/candidates` + report 0.3.0 三面原子，**已落 2026-08-27**）→ 渲染面 + K23 四语料 + K45 双腿 + 方法学册 13（0.3.0 第三键 `unmentioned_cut`、GUI 符号级下钻、`ce graph --mentions` 0.2.0 `rates` 普查、U 公式钉 + 258 行外部处置零 veto 缺陷，**已落 2026-08-28**）+ 自仓 101 条候选逐条处置；乙：19 条各一腿红/绿反事实（**已落 2026-08-29**，片 (a)(b)(c)）；丙：9 条就地改 + PreToolUse 全链实录数入 PERF-BUDGET + §2 复扫除新入场行外只变短（**已落 2026-08-29**，复扫例行随后经用户令退役）；收口判定式 87/87 具名（**复核 2026-08-29**：46 条已交付经 codex 四路只读审计逐条取证——35 落地 / 8 具名澄清 / 2 修补〔O42 类名出指纹、O43 注释〕/ 1 退役，41 条后置束计数与台账一致），台账 .ccm/frontier-2026-08-26；**v2.20 三步（用户四令 2026-08-29）**：更新功能三面等价（`ce update` 检查 = 退出码 0/1/2、`--yes` 就地替换 ce + ce-core，pin 来自 tag 上已提交的 `manifest.env`——O83 逆转，信任锚 = pin 提交而非签名；插件/cargo 所有的副本按名拒绝并指出各自的更新命令；GUI 第十屏 update 同一文档同一 apply 入口；插件 SessionStart 通知缓存一天 + `/codeeraser:update`；MCP 第十四工具 `update_check` 只到检查；`ce.update-report/0.1.0`；e2e 经 `file://` 端点 + `CE_UPDATE_TARGET_DIR` 隔离）→ 三面等价表由代码派生（clap 枚举 / Tauri 命令与屏 / MCP 目录 / hooks.json / plugin commands+skills）并逐字节门控（`face_parity`，具名省略写在表内）→ README 双语九节重构 + 仓内 demo（`demo/`：同一任务两跑只差 hook，七步真实判决，`demo_replay` 门）+ 全量文档同步（文档三律：中英分立 / 精简 / 全量）〔#16–#19 **已落 485f108**，2026-08-29；全量文档审计 14 面 61 条确认项同日就地修〕；**v2.21（2026-08-29 晚）**：ADR-009 文档事实派生 S1–S9（登记册 + 投影 + 五通道 + 引文文本锚 + bless 单读者，**已落 2026-08-29～30**：2b1ee4e / d4416d3 / b128c9e / 7d5c7e5 / b1d50fc + f758cfb / 086939c / 4257221 / 70e3deb）→ archify 架构图两张双语（e1ac748 钉版、ignored 缓存、`docs_diagrams` 门，**已落 6eba4cb**）→ README 双语更新 + 瘦身（190→145 行，**已落 d59134e**）→ 途中两件计划外根修：demo 收敛弧（每个循环跑到自己的终点，**已落 8c12f96**）、guard 双语（守卫全部句子收进 `guard/say.rs`，README 加两景展览，**已落 991b42f**）→ 站点八页（首页终端块改由测试生成、两语先量后写故为不动点，裸事实审计零发现，**已落 ab5871a 并已部署，线上 8/8 与 git blob 逐字节相同**）→ **v1.3.0 四渠道已发**（2026-08-30：pin 29007de → 投影 045a121 → tag **6da4463**〔首打被 verify-publish 按名拒 `build-macos=failure`，子仓 3e4645d 根修〕→ Release 十资产 + crates.io 1.3.0 + 官网 deploy + npm 1.3.0；发版后三轮自查全修 da4df74 / 194dd89 / 7e23591）→ **首页三张 GUI 图改由产品自己生成**（ADR-009 最后一个未派生面：`scripts/shoot_gui.js` 用无头 Edge——即应用自身的 WebView2 引擎——跑真 `gui/ui`，喂 CLI 出的三份报告文档，而那正是 webview 会收到的那份〔`ce … --format json` 与 `faces::*` 调同一个 `report_json`，跑在同一次 `judge::run` 上〕；`it/site_screenshots.rs` 五腿钉住 git 祖先关系〔界面动过而图没重拍即红〕、整窗尺寸、alt 文本不得手抄数字、收据 `contracts/gui-shots.json` 的 schema 等于代码当下的三个 `SCHEMA_ID`〔界面不动而报告形状动了的那条路，第一腿拦不住〕、`site/assets/` 每个文件都有门认领〔即本缺陷所属的类〕。手摆的旧图曾同时错报八格页签、`ce.join-report/0.1.0`、把已是子模块的 `cli/tests` 量在树内、以及 alt 文本里一个结构分数 854〔实为 832；与首页终端块的 953 不是同一个面，勿混〕；发版前自查又抓出取景器不可复现——每张图以点按钮开场而按钮有 0.12s 过渡，同一份报告集连拍三次出三个摘要，修法用应用自己就应答的 `prefers-reduced-motion`，第六腿 `it/site_shoot_motion.rs` 守住这对分处两文件的耦合，`gui-candidates.png` 随之重拍，收据只在拍进 `site/assets` 时才写） |
...
| R5 | 竞品挤压（**触发器式**，A8；**复扫例行已退役**——用户令 2026-08-29「不用管竞品」，本行封存） | 触发器 = 竞品发布**写入当轮拦截 + 克隆判定**两者兼备（2026-08-29 改写：原「diff 级 gating」已被 jscpd master / dupehound 达成而空位未失）→ 届时差异化收缩至三信号 join + 四分类、热路径查重改评估复用竞品引擎。**复扫 2026-08-29（v2.18 步 #15 O11，67-agent 核验工作流 + 亲核 README）：击发，带两条保留**——① jscpd master 2026-08-19 合入 `--baseline`/`--fail-on-new-clones`/`--baseline-from-ref`（commit 251544b/4a72a70），CHANGELOG 仍在 Unreleased、最新发版 v5.0.16（2026-08-18）不含；② 触发器未点名的新入场 dupehound 已**发布**变更函数级 diff 门（`check` 退 1 + MCP `check_duplication`，同栈 winnowing），但不做写入时拦截；desloppify 原句仍在；Claude Code 2.1.251 无内置查重（#10170、#34535 均 closed not-planned）。**裁定（用户委托取效果最好最优雅，2026-08-29）：击发-观察，路线不动**——三信号 join 早已随 H 批交付、四分类自 M4 在线，写入当轮 + join + 四分类的空位无人占；引擎复用不做：ADR-005 三条否决理由对 dupehound 逐字成立（批扫 CLI/MCP 进程、无常驻增量索引、外部运行时），dupehound 转 M 轮作 T1/T2 召回对拍方 |
