...
| **M2** 克隆热路径 + 进程模型 | winnowing 指纹索引（token 归一化覆盖全部五门首发语言——D2-4）、`ce dedup`、daemon（ADR-003 全项：懒启动/握手/WAL/冷启动降级） | 3 周 ± | 10 万 LOC 全量索引 < 30s；单文件增量 < 200ms；探针往返（含管道）p95 < 150ms；对 jscpd 可检出集召回 ≥ 95% **且**在同一真实仓库上精度 ≥ 90%（召回必配精度，B2）；property：增量 ≡ 全量重建 |
