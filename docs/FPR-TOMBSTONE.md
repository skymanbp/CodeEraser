# 墓碑残留度量 FPR 回放记录（2026-09-04，计划 v2.26 第一段 + v2.27 第二段步 2 / v2.28 步 7）

> 门（计划 v2.26 步 6）：500 次真实正常编辑误报 ≤ 1%；未达即停在 observe /
> feed-only；第二段 = 计划 v2.27（wire 族 `tombstone/1`、`[tombstone]` 档位表、册 14），步 2–7 已落，deny 晋级仍以本册为门。
...
| 9 | **6 / 9（0 + 9）** | 0 / 0 | v2.28 修正案：docdup 把连续 `///` 并成一段（`DOCDUP_REV` 5）——跨行的 Rust 文档句自此是一句，合取多成立三处：两处真阳（本批自己的散文追述被删的 `added_lines`，下一提交改写）+ 一处误报（单词名 `docs`，见下表）；不是本轮修掉的缺陷，是修 docdup 缺陷带来的口径变化 |
...
### 第九轮新增三处仲裁（`///` 合段后；原六处真阳原样，行号随合段移动：health.rs :48→:47、eval_graph_audit.rs :44→:42）

| # | 提交 | 站点 | 绑定的名字 | 判 | 理由 |
|---|---|---|---|---|---|
| 10 | 051a7301 | `cli/src/tombstone/surfaces.rs:142` | `added_lines` | 真阳 | codex 批删了 `added_lines` 后，新头注追述「the added lines joined alone once read … as one」；随下一提交改写 |
| 11 | 051a7301 | `docs/reference/methodology/14-…md:67` | `added_lines` | 真阳 | 册 14 同一句的散文版；同批改写 |
| 12 | ea494f46 | `cli/tests/eval_graph.rs:98` | `docs` | **误报** | 「a spec.rs or md.rs change used to invalidate all five docs」——`docs` 是普通名词（冻结文档），该提交恰好删了一个局部变量 `docs`；标记与名字此前分在两行、合段后进了同一句。单词名字按裁定继续算，这一处是其代价 |
...
| CodeEraser 自仓全史（**第九轮**，`///` 合段，历史多一提交） | 537 | 6 | 1 / 537 = **0.19 %** | 6 / 537 = **1.12 %** | 0.005 – 1.033 % / 0.411 – 2.416 % |
...
| 两语料合计（**第九轮**） | 937 | 6 | 1 / 937 = 0.11 % | 6 / 937 = 0.64 % | 0.003 – 0.593 % / 0.235 – 1.389 % |
...
**第九轮**（docdup `///` 合段后）严格读法 1 / 537 = 0.19 % 在线内；保守读法 6 / 537 = 1.12 % **超线一个提交**——多出的两处是真阳（规则受命要抓的、且是本批自己写的），如实记账、不挪线。
...
回放技巧（第八轮记，2026-09-04；第九轮 538 提交 / 537 事件同法）：release 构建先于回放，本机自仓全史约 3.5 分钟、requests 400 约 2 分钟；
