# 墓碑残留度量 FPR 回放记录（2026-09-04，计划 v2.26 第一段）
...
| 6 | 7 / 9（0 + 9） | 0 / 0 | —（终轮：剩余 9 处逐条仲裁见下表，没有可修的机制缺陷） |

changelog 定位豁免（`CHANGELOG.md` 路径见证）在每轮都做工：自仓 170 → 109 → 106 次（第三轮起
...
| CodeEraser 自仓全史 | 530 | 7 | 3 / 530 = **0.57 %**（折 /500 = 2.8） | 7 / 530 = 1.32 % |
| 两语料合计 | 930 | 7 | 3 / 930 = 0.32 % | 7 / 930 = 0.75 % |

**门（≤ 1 %）达成**：requests 两种读法都是 0；自仓按裁定尺度（出处叙事算残留）为 0.57 %；
只有把 4 个真阳提交也当误报的保守读法在自仓单独超线（1.32 %），而那 6 处正是规则受命要抓的。
第二段（wire 族、`ce.toml` 档位、册 14）据此可以另起 plan-set；本册所有数字由下方命令复现。

## 第二段要裁的两条口径

1. **计划书横幅算不算 changelog 定位**：三处中间态全出自 `docs/DEVELOPMENT_PLAN.md` 前四行的
   版本横幅——文档整体是计划，那几行是台账。两见证（路径、版本阶梯形）都不认它；要么给横幅一个
   显式见证，要么维持字面判（按裁定「除 changelog 类文档其他都算」，字面判是现行）。
2. **单词名字**：`legacy`、`truncate`、`nul`、`mentioned` 这类单词名字都在 3 字符地板之上、
   也不在词表里；6 处真阳里 4 处绑的是单词名，它们判对了，但把地板抬到两词窗口会漏掉它们。
