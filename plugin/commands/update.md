...
(no network, or the release's manifest could not be read). The last line names
the ONE action for this install; follow it exactly and do not substitute another:

- **plugin's bound copy** (the case under this command, since the starter binds
  `ce` by SHA256 pin): tell the user to run `/plugin update codeeraser` — the
  plugin's next manifest carries the new pins, and the starter re-verifies and
  re-downloads on the next session. Never run `ce update --yes` here: the
  starter would refuse a binary its manifest does not pin.
