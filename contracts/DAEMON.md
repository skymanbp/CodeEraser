...
- **超时与上限（本节 2026-08-25 补齐——此前只有字节上限入册、时间上限
  ...
  - **不对称，明写在此**：`ce` 给核的沉默设了期限、daemon 给对端的沉默
    设了期限，而**客户端对 daemon 的读没有期限**（`client.rs` 直接
    `read_line`）。卡死的 daemon 会把一次 PreToolUse 钩子无限期挂住。
    懒启动另有 **20 × 100 ms = 2 s** 的上线上限。已知缺口，在册不在实。
- **shutdown** → `bye` 后退出。
