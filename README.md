...
- **Interception at the instant of writing.** Every file's normalized tokens (identifiers → `ID`, literals → `LIT`, comments dropped) are winnowed with k = 25, w = 26, so any shared run of 50+ tokens is guaranteed a shared fingerprint. The fingerprints live in a SQLite WAL index kept by a lazy per-project daemon; the PreToolUse probe answers in <!--ce:restate:hook-probe:p50-ms:hook-probe#lead-->42<!--/ce--> ms p50 / <!--ce:restate:hook-probe:p95-ms:hook-probe#lead-->45<!--/ce--> ms p95 on a two-file fixture, and the whole plugin chain in 0.50 s p95. The guard charges only *novel* duplication: matches the replaced content already carried are subtracted, so on the live-stream reading it misfires on none of 719 production probes (0.00 per 500); the 2,761-event replay's full-file-write reading charges the 32 split-a-file intermediate states at 7.03 per 500 — both readings are ledgered in [FPR-REPLAY](docs/FPR-REPLAY.md).
...
- **Structure as a measured thing.** <!--ce:count:structure_axes#Word-->Eight<!--/ce--> axes (geometry, naming diversity, mixing, misplacement, conventions, stale docs, redundancy, modularity), Tsallis-2 entropy per directory, chi-squared divergence from a declared layout, and split-ROI pricing with four cost legs (crossing references, clone cuts, churn crossings, a new-file φ) or a cohesion alibi.
...
- **Tombstone residue, judged as a conjunction.** A change that erases a name and then narrates it — a struck-through or `(removed)` label, or a sentence carrying a backward-looking mark (`no longer`, `used to`, `曾经` …) together with the erased name — is measured in Rust as the changeset's surfaces (the names it erased, its new labels, its new sentences), judged in the core sentence by sentence (a mark and an erased name together; a label needs the name alone) against a floor and the class's `[tombstone] budget`, and spoken by three legs — PreToolUse, the Stop audit, `ce precommit` / `ce commitmsg` — only at the class's `[tombstone] tier` and only when the core answers `over`. Changelog-role documents are exempt by path, by ledger shape, by a segment with three version witnesses or by a `[tombstone] ledger` declaration, and the false-positive rate is replayed on git history before any default moves ([FPR-TOMBSTONE](docs/FPR-TOMBSTONE.md)).
...
**Installer.** Each [release](https://github.com/skymanbp/CodeEraser/releases) ships <!--ce:count:installers#word-->five<!--/ce--> GUI installers (NSIS `setup.exe` / AppImage / dmg) bundling the GUI, `ce` and the `ce-core` judgment core. The Windows installer puts the install dir on PATH and runs `ce setup`; AppImage and dmg users run `ce setup` once themselves — it finds Claude Code, wires the plugin below and says whether that `ce`'s directory is on PATH. The <!--ce:count:binaries#word-->fifteen<!--/ce--> binaries plus `SHA256SUMS` are unsigned by decision — verify with `sha256sum -c --ignore-missing SHA256SUMS`.

**Homebrew / winget.** From v1.7.0: `brew install skymanbp/codeeraser/codeeraser` (macOS and Linux, `ce` + `ce-core` from the pinned assets) and `winget install skymanbp.CodeEraser` (the Windows installer). The formula and the winget manifests under `packaging/` are generated from the same pinned manifest the release verified — never edited by hand.
...
| tree-scale structure (eight axes, split pricing) | `ce structure` | `structure`, `structure_report` | MCP `structure` |
...
| Claude Code wiring | `ce setup`, `ce setup --unwire` | — CLI only: the installers call it, AppImage / dmg users run it once | — |
...
- **One wire.** ce ↔ core is NDJSON over stdio with SemVer negotiation (proto <!--ce:ver:proto#v-->7.1.0<!--/ce-->, <!--ce:count:families#word-->twelve<!--/ce--> families); the per-project daemon speaks its own protocol (<!--ce:ver:daemon#v-->2.1.0<!--/ce-->) over `interprocess`; a protocol-major skew is a named refusal, never a guess.
...
- **Philosophy.** Measure in Rust, decide in Haskell, render everywhere else. Codes cross the wire; each face owns its sentences. Nothing on any surface asks a model anything. Hooks fail open and say so. A guard class reaches `deny` only with its own false-positive record in [CHANGELOG](CHANGELOG.md). Documentation is generated or gated: the CLI and config references, the <!--ce:count:booklets#word-->fifteen<!--/ce-->-booklet [methodology](docs/reference/methodology.md) with machine-checked citations, the numbers this page derives from the code, the two diagrams above, the bench block, the demo, the site's terminal block and its GUI screenshots, the parity table, the NOTICE. This repository is its own first user — every push runs the <!--ce:count:gates#word-->six<!--/ce--> product gates on this tree.

## Known limits

**v1.7.0 scope.** The v2.29 plan revives selected deferred work for v1.7.0: the three bundles the plan's K–L row named — scoring and evaluation, distribution, and the evidence gates that would promote guard classes — were ruled not-done on 2026-08-31 (plan v2.22, 45 items) and re-opened item by item on 2026-09-05 where the gain covers the cost. The limits below describe the current release.

**Limits.** PreToolUse shapes behaviour; it is not a security wall (shell writes bypass it — the Stop audit and CI are the backstops). Hooks fail open on internal errors and record the degradation. AST-based judgment uses the <!--ce:count:grammars#word-->six<!--/ce--> grammars above; Markdown is judged by the documentation and graph rules without a tree-sitter grammar; JSDoc and Rust `///` are comments, not docstrings; T4 clones are not promised. `churn`, `join` and `trend` are minute-scale. Binaries are unsigned. Symbol-level liveness is an advisory and never a verdict — `ce deadcode` says so on its own last line. A guard class stays `observe` until it carries its own false-positive record. The complexity axis ships with no hard line at all — `cognitive_fail` defaults to 0, so a tangled function warns and nothing more until a repository declares one, and the write-time hook never judges complexity. `ce structure` has no score floor, so that family reports and gates nothing. A release builds <!--ce:count:platforms#word-->five<!--/ce--> targets since v1.7.0 (`x86_64-windows`, `x86_64-linux`, `aarch64-macos`, `x86_64-macos`, `aarch64-linux`); on the two newer ones a plugin starter reading a manifest older than that finds an empty pin and falls back to a PATH `ce` or a source install. `ce setup` wires nothing when the account running it is not the logged-in user (exit 13) — run it unelevated from your own account. Judging this repository needs the `cli/tests` submodule seated (it is a reader of the tree, never a measured part). Tombstone residue counts single-word names — a common word erased as an identifier can bind a sentence genuinely about that word, and the repository's own `[tombstone] terms` list is the way out — and measures CJK names only at word boundaries: a wide Chinese name is seen where it stands alone as a heading, a list lead or an identifier, and is bound in prose by substring. Scores are not comparable across a `[[rules.class]]` switch, across the v0.7.3 → v1.0.0 density-charge rescoring, across the v1.2.0 → v1.3.0 test-submodule move, across the v1.3.x → v1.4.0 recursion increment, or across the v1.6.0 → v1.7.0 re-denomination of the clone and docdup axes (files touched over each axis's own universe, with docdup pairs reaching `ce check` for the first time). They are also not comparable across a named re-establish that moves the baseline's `softLine`: the size axis charges against that repo-relative line rather than a constant, and this repository's has walked from 304 (v0.7.3) to 372 (v1.4.1) — three points apart when both lines are applied to the v1.4.1 tree.
