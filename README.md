...
[![Architecture: the repository is parsed and fingerprinted by the Rust measurement side (tree-sitter, the SQLite fingerprint index kept warm by a per-project daemon, the reference graph, git windows), crosses one NDJSON wire of twelve families to the Haskell judgment core with its policy shipped as data, and the same reports are rendered by five faces — terminal, GUI, MCP server, Claude Code hooks, CI](docs/assets/architecture.en.svg)](docs/assets/architecture.en.svg)
