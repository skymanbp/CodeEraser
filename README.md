...
**Scope.** Judged languages: Python, TypeScript/TSX, Rust, Go, Haskell and Markdown (seven language codes over six tree-sitter grammars). Size-only arm: js/mjs/cjs/jsx, css/scss/less, html/htm, vue, svelte, sh/bash, yml/yaml — they enter the size gates, the hard budget and the ratchet, never a semantic verdict. Faces: the `ce` CLI, the GUI (ten screens), the Claude Code plugin (three hooks, one skill, one command, fourteen read-only MCP tools), pre-commit and CI.
