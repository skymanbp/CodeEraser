...
The two denied writes are the two copies of an existing helper; the compact renderer that slipped past is the honest boundary — a full-file rewrite copies its own blocks, so nothing is *novel* at write time, and the next two layers, the Stop audit and the CI clone budget, catch it. Both transcripts, both SVGs and the JSON behind this table are generated by [`demo/run.js`](demo/README.md) and re-checked byte for byte in CI (`demo_replay`). The first real interception, recorded the day it happened: [T1-INTERCEPT](docs/T1-INTERCEPT.md).
...
Console output and `--help` are English by default and Chinese under `--lang zh` (or `CE_LANG=zh`); JSON and the FAIL/pass vocabulary are never translated. `[[rules.class]]` in `ce.toml` gives one glob set its own size and complexity lines and ratchet tolerance (`0` = may not grow), and the same line is read by the score, the `ce scan` ladder and the PreToolUse budget ([ce.toml reference](docs/reference/ce-toml.md)).
